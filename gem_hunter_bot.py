...
GEM_VOL_MAX = 150_000     # max 150K USDT/24h — perlebar cakupan microcap
...
MTF_ENABLED         = True   # aktifkan konfirmasi 1h & 1d
MTF_RSI_1H_MAX      = 72     # RSI 1h tidak boleh overbought
MTF_RSI_1D_MIN      = 30     # RSI 1d tidak boleh oversold parah
MTF_RSI_1D_MAX      = 75     # RSI 1d tidak boleh overbought

# ── Liquidity / Spread Filter ──────────────────────────────────────
LIQ_SPREAD_MAX_PCT  = 3.0    # spread bid-ask maksimal 3% — lebih = illikuid
LIQ_ENABLED         = True   # aktifkan filter likuiditas
...
def check_mtf(client: SpotApi, pair: str) -> dict:
    """
    Konfirmasi sinyal dengan timeframe 1h dan 1d.
    Returns dict dengan hasil konfirmasi dan bonus score.
    """
    result = {
        "ok": True,
        "rsi_1h": 50.0,
    ...
    try:
        ...
        if raw_1h and len(raw_1h) >= 15:
            ...
            if rsi_1h > MTF_RSI_1H_MAX:
                ...
                result["mtf_bonus"] += 1
                result["detail"].append(f"RSI 1h sehat ({rsi_1h:.0f})")
        ...
        if raw_1d and len(raw_1d) >= 10:
            ...
            result["trend_1d"] = price_now > ema20_1d
            ...
            if price_now > ema20_1d:
                result["mtf_bonus"] += 1
                result["detail"].append("Harga di atas EMA20 daily ✓")
            else:
                result["detail"].append("Harga di bawah EMA20 daily")

    except Exception as e:
        log(f"MTF check [{pair}]: {e}", "warn")

    return result


def check_liquidity(client: SpotApi, pair: str) -> dict:
    """
    Cek likuiditas via spread bid-ask dari orderbook.
    Spread > 3% = illikuid, skip sinyal.
    """
    result = {"ok": True, "spread_pct": 0.0}
    if not LIQ_ENABLED:
        return result
    ...
    try:
        ...
        if ob and ob.bids and ob.asks:
            ...
            if best_bid > 0:
                ...
                if spread_pct > LIQ_SPREAD_MAX_PCT:
                    result["ok"] = False
    except Exception as e:
        log(f"Liquidity check [{pair}]: {e}", "warn")
...
def analyze_gem(
    ...
    # ── Gate 9: Multi-Timeframe Confirmation ─────────────────────
    mtf = check_mtf(client, pair)
    if not mtf["ok"]:
        return None  # RSI 1h overbought atau RSI 1d di luar range

    mtf_bonus = mtf["mtf_bonus"]  # 0–3

    # ── Gate 10: Liquidity / Spread Check ────────────────────────
    liq = check_liquidity(client, pair)
    if not liq["ok"]:
        return None  # spread terlalu lebar — koin illikuid
    ...
    total_score = (
        ...
        + ath_score       # 0–3
        + ema_score       # 0–2
        + mtf_bonus          # 0–3 (NEW: konfirmasi 1h & 1d)
    ...
    return {
        ...
        "rsi_1h":         round(mtf["rsi_1h"], 1),
        "rsi_1d":         round(mtf["rsi_1d"], 1),
...
def format_signal(sig: dict) -> str:
    ...
    return (
        ...
        f"Trend 1d        : {'✅ Di atas EMA20' if sig['trend_1d'] else '⚠️ Di bawah EMA20'}\n"
        f"Spread          : <b>{sig['spread_pct']:.2f}%</b>  |  24h: {sig['change_24h']:+.1f}%\n"
