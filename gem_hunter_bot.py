"""
╔══════════════════════════════════════════════════════════════════╗
║            GEM HUNTER BOT — Standalone v3.1 "Institutional Sniper" ║
║                                                                  ║
...
║  [S1-FREQ]  Trade Frequency Filter: deteksi lonjakan jumlah      ║
║             transaksi per menit (bukan hanya volume). Koin tidur ║
║             biasanya 1 tx/menit — jika tiba-tiba 20+ tx/menit,  ║
║             itu tanda bot exchange lain mulai "bocor" ke Gate.io ║
║  [S2-MCAP]  Micro Market Cap Scanner: filter harga rendah +      ║
║             estimasi supply untuk fokus pada koin est. MCap      ║
║             <$5 Juta. Label "ULTRA GEM 💎💎" di Telegram jika   ║
║             est. MCap di bawah threshold. Hanya koin super kecil ║
║             yang bisa naik 100x.                                 ║
║  [S3-WALL]  Volume Wall Detection: cek konsistensi antara volume ║
║             breakout vs tebal Buy Wall di order book. Volume     ║
║             meledak tanpa bid wall = Fake Pump, diblokir.        ║
║             Volume + bid wall tebal = sinyal Strong Buy.         ║
║                                                                  ║
...
GEM_FLAT_MIN_LEN  = 48    # [F2-DORM] minimum 48 candle × 15m = 12 jam
...
GEM_HIGH90D_CANDLE_LIMIT = 8640   # [F1-SPEED] 8640 × 15m = 90 hari (ganti dari 2160 × 1h)
...
# [F1-SPEED] 2880 × 15m = 30 hari (ganti dari 180 × 4h)
VOL30_CANDLE_LIMIT = 2880   # [F1-SPEED] 2880 × 15m = 30 hari
...
# [F1-SPEED] 288 × 15m = 3 hari minimum; 2688 × 15m ≈ 28 hari untuk flag
NEW_LISTING_MIN_CANDLES_4H = 288   # [F1-SPEED] 3 hari × 96 candle/hari = 288 candle 15m
...
GEM_FLAT_MIN_LEN_BTC_BLOCK  = 96      # [F2-DORM] 96 × 15m = 24 jam, lebih ketat saat BTC drop
...
def get_high_90d(client: SpotApi, pair: str, fallback: float) -> float:
    """
    [F1-SPEED] Fetch 8640 candle 15m = 90 hari untuk mendapatkan puncak harga terkini.
    (Sebelumnya menggunakan 2160 candle 1h)
    ...
    try:
        ...
        if raw and len(raw) >= 96:   # minimal 1 hari data (96 × 15m)
...
def check_mtf(client: SpotApi, pair: str, closes_4h: np.ndarray | None = None) -> dict:
    ...
    try:
        ...
        if raw_5m and len(raw_5m) >= 15:
            ...
            result["rsi_1h"] = rsi_5m   # field tetap bernama rsi_1h untuk backward compat
            if rsi_5m > MTF_RSI_1H_MAX:
                result["ok"] = False
        ...
        if raw_1h and len(raw_1h) >= 10:
            ...
            result["rsi_1d"]   = rsi_1h   # field tetap bernama rsi_1d untuk backward compat
            result["trend_1d"] = price_now > ema(closes_1h, 20)
            if not (MTF_RSI_1D_MIN <= rsi_1h <= MTF_RSI_1D_MAX):
                result["ok"] = False
            ...
            if result["trend_1d"]:
                result["mtf_bonus"] += 1
            # TF alignment: 1h
            if result["trend_1d"] and rsi_1h > 45:
                result["tf_alignment"] += 1
...
def check_volume_wall(client: SpotApi, pair: str, candle_vol_median: float) -> dict:
    ...
    try:
        ...
        if candle_vol_median > 0:
            ...
            result["wall_legit"] = (
                wall_usdt >= OB_WALL_MIN_USDT and
                wall_ratio >= OB_WALL_VOL_RATIO
            )

            if result["wall_legit"]:
                result["wall_score"] = 2 if wall_ratio >= OB_WALL_VOL_RATIO * 3 else 1
            elif wall_usdt < OB_WALL_MIN_USDT * 0.3:
                # Wall sangat tipis — kemungkinan fake pump / wash trading
...
def run_gate_filters(
    ...
    # Gate 4: Volume spike [F3-VOL]
    vol_baseline = float(np.mean(fv))   # [F3-VOL] kembali ke np.mean() — lebih sensitif terhadap pergerakan awal
    if vol_baseline <= 0:
        return None
    # [F3-VOL] Ganti np.percentile(75) → np.mean() pada breakout window.
    # Tujuannya berbeda dari v2.9: kita ingin teriakkan alarm saat volume mulai "panas",
    # bukan saat volume sudah "meledak". Mean lebih responsif terhadap kenaikan awal
    # dibanding percentile yang masih bisa tertahan oleh candle sepi di awal window.
    ...
    # Gate 12b: Volume Wall Consistency [S3-WALL]
    # Cross-validate volume spike vs buy wall — tolak jika wall sangat tipis
    # (indikasi kuat wash trading / fake pump).
...
def compute_scores(
    ...
    # Scoring max = 32 (base) + 2 (freq) + 1 (mcap) + 2 (wall) = 37
    # GEM_TIER threshold tidak diubah agar backward compat — skor baru bersifat additive bonus
    total_score = (
...
def build_signal(
    ...
    # [S2-MCAP] Upgrade tier display jika ULTRA GEM terdeteksi
    micro_mcap    = scores["micro_mcap"]
    if micro_mcap["is_ultra_gem"] and tier in ("GEM", "WATCH"):
        tier = "MOONSHOT"   # auto-upgrade: koin ultra micro lebih pantas masuk MOONSHOT
    ...
    return {
        ...
        "ath_dist_pct": round(gate["high90d_dist"] * 100, 1),
...
def format_signal(sig: dict) -> str:
    ...
    score_d = (
        ...
        f"· Freq {sig.get('freq_score',0)}/2 · MCap {sig.get('is_ultra_gem',False) and 1 or 0}/1 · Wall {sig.get('wall_score',0)}/2"
    )
    ...
    return (
        ...
        f"RSI 15m : <b>{sig['rsi']}</b>  |  5m: {sig['rsi_1h']}  |  1h: {sig['rsi_1d']}\n"
        f"TF Align: {'✅✅✅' if sig['tf_alignment']==3 else '✅✅⬜' if sig['tf_alignment']==2 else '✅⬜⬜'}  "
        f"Trend 1h: {'✅' if sig['trend_1d'] else '⚠️'}\n"
        f"MCap    : <i>{sig['mcap_tier']}</i>  |  24h: {sig['change_24h']:+.1f}%\n"
        f"Spread  : <b>{sig['spread_pct']:.2f}%</b>  |  Score: <b>{sig['score']}/37</b>\n"
        f"<i>({score_d})</i>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
...
def format_summary(scanned: int, skipped: int, found: int, sent: int,
    ...
    if halted:
        return f"🛑 <b>GEM HUNTER v3.1 — HALT</b>\nBTC crash. Scan dilewati.\n<i>{now}</i>"
    return (
        f"💎 <b>GEM HUNTER v3.1 Institutional Sniper Scan Selesai</b>\n━━━━━━━━━━━━━━━━━━━━\n"
...
def run_scan() -> None:
    log("=" * 55)
    log(f"💎 GEM HUNTER v3.1 Institutional Sniper — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')}")
    log("=" * 55)
