...
║  [FIX-E8]  Minimum candle volume median $200/4h candle;          ║
...
# ── Candle ────────────────────────────────────────────────────────
GEM_TF           = "4h"
GEM_CANDLE_LIMIT = 168

# ── Dormancy ──────────────────────────────────────────────────────
GEM_FLAT_WINDOW   = 60
GEM_FLAT_MAX_BODY = 2.0   # [FIX-E5] diperketat 3.0 → 2.0; koin 2.8% range tidak benar-benar dormant
GEM_FLAT_MIN_LEN  = 10

# ── Breakout ──────────────────────────────────────────────────────
GEM_BREAKOUT_WINDOW    = 6
GEM_VOL_SPIKE_MIN      = 8.0
GEM_BREAKOUT_PCT_MIN   = 4.0
...
GEM_HIGH90D_CANDLE_LIMIT = 2160   # 2160 × 1h = 90 hari
...
GEM_CANDLE_VOL_MIN    = 200.0 # [FIX-E8] median volume per candle 4h minimum $200 USDT

# ── Scoring Tiers [UPG-C3] — max score 32 ────────────────────────
# Breakdown: dormancy(3)+vol(3)+breakout(3)+rsi(2)+ath(3)+ema(2)
#            +mtf_bonus(3)+tf_alignment(3)+sweep(2)+macd(1)
#            +depth(2)+sector(2)+vol30(2)+narrative(1) = 32
GEM_TIER: dict[str, int] = {"MOONSHOT": 22, "GEM": 16, "WATCH": 11}
...
# ── 30-Day Volume Anomaly [UPG-C1] ────────────────────────────────
VOL30_CANDLE_LIMIT = 180   # 180 × 4h = 30 hari
...
PUMP_CANDLE_BODY_PCT  = 28.0   # [FIX-E3] dinaikkan 15% → 28%; 15% terlalu rendah untuk small-cap dormant breakout
...
NEW_LISTING_MIN_CANDLES_4H = 18   # 3 hari minimum
NEW_LISTING_MAX_DAYS       = 14   # flag display only (bukan bonus score)

# ── Flat Base Slope Filter [FIX-B7] — lebih ketat dari v2.1 ───────
FLAT_DOWNTREND_SLOPE_MAX    = -0.05   # min -0.05% per candle (≈ -3% drift per 60 candle) [A2: unit = %/candle, bukan total drift]
GEM_FLAT_MIN_LEN_BTC_BLOCK  = 15      # [WEAK-2] lebih ketat saat BTC sedang drop
...
# ══════════════════════════════════════════════════════════════════
#  90-DAY HIGH DETECTION [FIX-2, A2]
# ══════════════════════════════════════════════════════════════════

def get_high_90d(client: SpotApi, pair: str, fallback: float) -> float:
    """
    [FIX-2] Fetch 2160 candle 1h = 90 hari untuk mendapatkan puncak harga terkini.

    [A2] PENTING — Ini BUKAN All-Time High (ATH) sejati.
    Fungsi ini hanya mengembalikan harga tertinggi dalam 90 hari terakhir.
    ...
    "ruang ke puncak 90 hari yang observable".
    ...
    try:
        raw = gate_retry(client.list_candlesticks, currency_pair=pair,
                         interval="1h", limit=GEM_HIGH90D_CANDLE_LIMIT)
        if raw and len(raw) >= 24:
            return float(np.max([float(c[3]) for c in raw]))
    except Exception as e:
        log(f"get_high_90d [{pair}]: {e}", "warn")
    return fallback
...
def check_mtf(client: SpotApi, pair: str, closes_4h: np.ndarray | None = None) -> dict:
    """
    [UPG-3] TF alignment score sekarang dihitung dalam check_mtf
    menggunakan data yang sudah di-fetch — zero extra API calls.
    """
    result = {
        "ok": True, "rsi_1h": 50.0, "rsi_1d": 50.0,
        "trend_1d": True, "mtf_bonus": 0, "tf_alignment": 0, "detail": [],
    }
    if not MTF_ENABLED:
        return result

    # TF alignment: 4h (dari closes yang sudah di-fetch di analyze_gem)
    if closes_4h is not None and len(closes_4h) >= 20:
        if closes_4h[-1] > ema(closes_4h, 20) and rsi(closes_4h) > 45:
            result["tf_alignment"] += 1

    # [MEDIUM-4] Track apakah masing-masing TF berhasil di-fetch.
    # Jika fetch gagal (gate_retry return None), koin tidak boleh lolos MTF secara default.
    fetched_1h = False
    fetched_1d = False

    try:
        raw_1h = gate_retry(client.list_candlesticks, currency_pair=pair, interval="1h", limit=50)
        if raw_1h and len(raw_1h) >= 15:
            ...
            if rsi_1h > MTF_RSI_1H_MAX:
                result["ok"] = False
                result["detail"].append(f"RSI 1h overbought ({rsi_1h:.0f})")
            elif rsi_1h < 55:
                result["mtf_bonus"] += 1
            # TF alignment: 1h
            if closes_1h[-1] > ema(closes_1h, 20) and rsi_1h > 45:
                result["tf_alignment"] += 1

        raw_1d = gate_retry(client.list_candlesticks, currency_pair=pair, interval="1d", limit=30)
        if raw_1d and len(raw_1d) >= 10:
            fetched_1d     = True
            closes_1d      = np.array([float(c[2]) for c in raw_1d], dtype=float)
            rsi_1d         = rsi(closes_1d)
            price_now      = float(closes_1d[-1])
            result["rsi_1d"]   = rsi_1d
            result["trend_1d"] = price_now > ema(closes_1d, 20)
            if not (MTF_RSI_1D_MIN <= rsi_1d <= MTF_RSI_1D_MAX):
                result["ok"] = False
                result["detail"].append(f"RSI 1d di luar range ({rsi_1d:.0f})")
            else:
                result["mtf_bonus"] += 1
            if result["trend_1d"]:
                result["mtf_bonus"] += 1
            # TF alignment: 1d
            if result["trend_1d"] and rsi_1d > 45:
                result["tf_alignment"] += 1

    except Exception as e:
        log(f"MTF [{pair}]: {e}", "warn")

    # [MEDIUM-4] Jika salah satu TF gagal di-fetch, tolak koin — jangan biarkan lolos
    # karena API down atau rate-limited. Sinyal tanpa konfirmasi MTF lebih berbahaya.
    if not fetched_1h or not fetched_1d:
        result["ok"] = False
        missing = []
        if not fetched_1h: missing.append("1h")
        if not fetched_1d: missing.append("1d")
...
def get_btc_regime(client: SpotApi) -> dict:
    """
    [FIX-B3] v2.1 hanya membandingkan 1 candle sebelumnya — sangat noise.
    v2.2 pakai rolling 7-candle (1h) dan 5-candle (4h) untuk representasi
    tren yang lebih stabil dan tidak mudah terpicu oleh 1 wick.

    [A5] Exception handler diubah ke fail-safe: jika data BTC tidak tersedia
    (exchange down, rate limit total), return halt=True. Sebelumnya return
    halt=False → scan berlanjut padahal kondisi market tidak dapat diverifikasi.
    Ini secara konseptual salah: ketidakpastian harus default ke aman (halt),
    bukan default ke lanjut.
    """
    ...
    try:
        ...
        if c1h and len(c1h) >= 7:
            ...
            # [A5] Data 1h tidak tersedia — fail-safe
        ...
        if c4h and len(c4h) >= 5:
            ...
            # [A5] Data 4h tidak tersedia — fail-safe
        ...
        # [A5] Fail-safe: exception saat ambil data BTC = kondisi exchange tidak normal.
        # Lebih aman halt daripada scan dengan asumsi pasar normal.
...
def save_signal(sig: dict) -> None:
    ...
    try:
        sb().table("gem_signals").insert({
            ...
            "avg_flat_range": sig["avg_flat_range"], "ath_dist_pct": sig["ath_dist_pct"],
            ...
            "candle_vol_median": sig["candle_vol_median"],  # [ISU-1] median vol per candle 4h
...
def calc_vol30_score(
    client: SpotApi, pair: str, current_vol_ratio: float,
    ...
    [MEDIUM-3] v2.7 menerima prefetched_volumes jika >= 60 candle, padahal
    prefetched adalah 168 candle 4h = 7 hari, bukan 30 hari. Threshold anomaly
    50× dan 20× dihitung dari baseline 7 hari → lebih mudah trigger palsu.

    v2.8: hanya gunakan prefetched jika >= 150 candle (~25 hari, mendekati 30 hari).
    Jika kurang, selalu fetch 180 candle terpisah untuk baseline 30 hari yang benar.
    Trade-off: +1 API call per pair yang lolos gate awal, tapi akurasi baseline terjaga.
    """
    try:
        # Gunakan prefetched hanya jika mendekati 30 hari (>= 150 candle ≈ 25 hari)
        if prefetched_volumes is not None and len(prefetched_volumes) >= 150:
            vols_30d = prefetched_volumes
        else:
            # [MEDIUM-3] Fetch terpisah untuk memastikan baseline 30-hari yang benar
            raw = gate_retry(client.list_candlesticks, currency_pair=pair,
                             interval=GEM_TF, limit=VOL30_CANDLE_LIMIT)
            if not raw or len(raw) < 60:
                return 0
            vols_30d = np.array([float(c[1]) for c in raw], dtype=float)

        baseline_30 = float(np.median(vols_30d[:-6]))   # median 30d, kecuali 6 candle terakhir
        if baseline_30 <= 0:
            return 0
        # Vol terkini = max 6 candle terakhir (konsisten dengan vol_ratio)
...
def run_gate_filters(
    ...
    if n < NEW_LISTING_MIN_CANDLES_4H:
        return None
    is_new_listing = n < (NEW_LISTING_MAX_DAYS * 6)

    flat_w = GEM_FLAT_WINDOW if n >= GEM_FLAT_WINDOW + GEM_BREAKOUT_WINDOW + 5 \
             else max(n - GEM_BREAKOUT_WINDOW - 3, GEM_FLAT_MIN_LEN)
    # [WEAK-2] Lebih ketat saat BTC sedang drop — kurangi false breakout
    ...
    # Gate 3b: Slope filter [FIX-B7]
    flat_slope = linear_slope(fc)
    if flat_slope < FLAT_DOWNTREND_SLOPE_MAX:
        return None

    # Gate 4: Volume spike [FIX-3]
    vol_baseline = float(np.median(fv))
    if vol_baseline <= 0:
        return None
    # [A1] Ganti np.max → np.percentile(75): satu candle anomali (fat-finger,
    # bot artefact) tidak lagi bisa sendirian memenuhi threshold 8×.
    # Percentile ke-75 dari 6 candle breakout window lebih merepresentasikan
    # distribusi volume sesungguhnya daripada nilai ekstrem tunggal.
    vol_ratio = float(np.percentile(volumes[-GEM_BREAKOUT_WINDOW:], 75)) / vol_baseline
    if vol_ratio < GEM_VOL_SPIKE_MIN:
        return None

    # Gate 5: Candle vol minimum [FIX-E8]
    ...
    # Gate 7: Early breakout pct + dist from base
    ...
    # Gate 10: MTF (sebelum 90d high — lebih ringan, hemat API call) [MINOR-9]
    mtf = check_mtf(client, pair, closes_4h=closes)
    if not mtf["ok"]:
        return None

    # Gate 11: 90-Day High — bukan ATH sejati, lihat docstring get_high_90d [A2]
...
def compute_scores(
    ...
    vol30_score         = calc_vol30_score(client, pair, gate["vol_ratio"], prefetched_volumes=volumes)
...
def build_signal(
    ...
    if   total_score >= GEM_TIER["MOONSHOT"]: tier = "MOONSHOT"
    elif total_score >= GEM_TIER["GEM"]:      tier = "GEM"
    elif total_score >= GEM_TIER["WATCH"]:    tier = "WATCH"
    else: return None
    ...
    return {
        ...
        # [A2] Renamed: ath_dist_pct → high90d_dist_pct agar tidak mislabeled sebagai ATH
        "high90d_dist_pct": round(gate["high90d_dist"] * 100, 1),
        "ath_dist_pct": round(gate["high90d_dist"] * 100, 1),   # backward compat untuk save_signal
        ...
        "rsi_1h": round(gate["mtf"]["rsi_1h"], 1),
        "rsi_1d": round(gate["mtf"]["rsi_1d"], 1),
        "trend_1d": gate["mtf"]["trend_1d"],
...
def analyze_gem(
    ...
    return build_signal(pair, price, vol_24h, change_24h, gate, scores)
...
def format_signal(sig: dict) -> str:
    ...
    if sig["tf_alignment"] == 3:
        ctx.append("🎯 Full TF alignment (1h+4h+1d bullish) — konfluensi kuat")
    ...
    return (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Vol spike    : <b>{sig['vol_ratio']:.1f}×</b> median flat  |  <b>{sig['vol30_score']}/2</b> vs 30d\n"
        f"📈 Breakout     : <b>+{sig['breakout_pct']:.1f}%</b> dari base  |  Candle vol: ${sig.get('candle_vol_median', 0):.0f}/4h\n"
        f"💤 Dormancy     : {sig['avg_flat_range']:.1f}% avg range\n"
        f"📉 90d High dist : <b>{sig['high90d_dist_pct']:.0f}%</b> di bawah puncak 90d\n"
        f"📚 OB Depth     : {sig['depth_ratio']:.1f}× bid/ask  |  Sektor: {sig['sector']}\n"
        f"RSI 4h  : <b>{sig['rsi']}</b>  |  1h: {sig['rsi_1h']}  |  1d: {sig['rsi_1d']}\n"
        f"TF Align: {'✅✅✅' if sig['tf_alignment']==3 else '✅✅⬜' if sig['tf_alignment']==2 else '✅⬜⬜'}  "
        f"Trend 1d: {'✅' if sig['trend_1d'] else '⚠️'}\n"
        f"MCap    : <i>{sig['mcap_tier']}</i>  |  24h: {sig['change_24h']:+.1f}%\n"
        f"Spread  : <b>{sig['spread_pct']:.2f}%</b>  |  Score: <b>{sig['score']}/32</b>\n"
        f"<i>({score_d})</i>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
...
def check_outcomes(client: SpotApi) -> int:
    ...
    try:
        res = (
            ...
            .select("id,pair,entry,tp1,tp2,tp3,sl,vol_ratio,flat_vol_median,created_at,high_watermark,vol_weak_alerted")  # [A4] vol_ratio; [MEDIUM-6] flat_vol_median dari DB
            .is_("outcome", "null")
...
def run_scan() -> None:
    log("=" * 55)
    log(f"💎 GEM HUNTER v2.9 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')}")
    log("=" * 55)
    ...
    for t in tickers:
        ...
        try:
            ...
            if not (GEM_VOL_MIN <= vol_24h <= GEM_VOL_MAX):
                skipped += 1
                continue
            # [FIX-B4] Cek dari cache, bukan query Supabase
            ...
            if sig:
                ...
                log(
                    f"  ✔ {pair} [{sig['tier']}] score={sig['score']}/32 "
    ...
    for sig in candidates:
        ...
        log(f"  📤 {sig['pair']} [{sig['tier']}] score={sig['score']}/32", "ok")
        sent += 1
        time.sleep(0.5)
