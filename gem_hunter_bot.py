"""
╔══════════════════════════════════════════════════════════════════╗
║            GEM HUNTER BOT — Standalone v3.9 "10/10 Final"       ║
║                                                                  ║
...
║                                                                  ║
║  Changelog v3.9 (dari v3.8) — 10/10 FINAL (COSMETIC FIXES):     ║
║  [I1-NAME]  calc_vol30_score(): variabel 'vols_30d', 'baseline_30'║
║             dan 'ratio_30d' di-rename ke 'vols_buf', 'baseline',  ║
║             dan 'ratio' — window sebenarnya 10 hari (384 × 15m), ║
║             bukan 30 hari. Docstring dan komentar dikoreksi.     ║
║  [I2-LOG]   gate_retry() log structure dikoreksi: log kedua      ║
║             dipindah ke else-branch eksplisit agar jelas bahwa   ║
║             ia hanya jalan untuk status selain 429/500-503/400.  ║
║             Mencegah kebingungan "apakah ada duplicate log?"     ║
║                                                                  ║
║  Changelog v3.8 (dari v3.7) — PERFECT SCORE (PROD FIXES):       ║
║  [H1-HIGH]  get_high_90d() multi-batch _from/to parameter        ║
║             dihapus — Gate.io SDK tidak support param ini,       ║
║             menyebabkan 400 Bad Request × 4 per koin qualifying. ║
║             Diganti: single fetch limit=1000 + additional        ║
║             fallback ke np.max(highs) dari candle yang sudah     ║
║             di-fetch di gate sebelumnya. Zero wasted API calls.  ║
║  [H2-HTML]  ctx "<14 hari" di-escape jadi "&lt;14 hari" agar     ║
║             Telegram HTML parser tidak gagal saat ada new listing ║
║             koin yang lolos. Berpotensi error 400 seperti kasus  ║
║             $150K kemarin jika tidak difix.                      ║
║  [H3-HTML]  Audit menyeluruh semua string yang dikirim ke tg():  ║
║             semua bare '<' dan '&' di-escape konsisten.          ║
║  [H4-GATE]  gate_retry menambahkan penanganan khusus 400:        ║
║             log pesan lebih informatif membedakan "param invalid" ║
║             vs error lain. Tidak retry (benar), tapi log jelas.  ║
║  [H5-DOC]   SCAN_SLEEP_SEC env override didokumentasikan         ║
║             di komentar konstanta beserta contoh nilai.          ║
║                                                                  ║
...
# ── Scan ──────────────────────────────────────────────────────────
# [H5-DOC] SCAN_SLEEP_SEC dapat di-override via GitHub Actions secrets.
# Contoh nilai yang direkomendasikan:
#   0.20 = agresif (prime time only, risiko rate limit tinggi)
#   0.30 = balanced (override populer, ditemukan di log produksi)
#   0.40 = default off-peak (aman, hemat rate limit)
    ...
    for attempt in range(retries):
        try:
            ...
            if e.status == 429:
                ...
                # [H4-GATE] 400 = Bad Request — parameter tidak valid, tidak perlu retry
                log(f"Gate 400 Bad Request: {e.reason} — skip (param invalid)", "warn")
                return None
            else:
                # [I2-LOG] Explicit else — jalan hanya untuk status selain 429/500-503/400
...
GEM_HIGH90D_FETCH_BATCHES = 1   # [H1-HIGH] v3.8: multi-batch dihapus — _from/to param
                                 # tidak disupport Gate.io SDK (menyebabkan 400 Bad Request).
                                 # Single fetch limit=1000 (≈10 hari) sudah cukup sebagai
                                 # referensi "puncak observable terkini". Konstanta dipertahankan
                                 # untuk backward compat komentar changelog.

def get_high_90d(client: SpotApi, pair: str, fallback: float) -> float:
    """
    [H1-HIGH] v3.8: Multi-batch _from/to fetch DIHAPUS.
    Gate.io Python SDK tidak mendukung parameter _from dan to di
    list_candlesticks() — setiap batch call menghasilkan 400 Bad Request,
    membuang 4 API calls sebelum fallback. Terdeteksi dari log produksi.

    v3.8: Single fetch limit=1000 (≈10 hari coverage di 15m TF).
    Jika fetch gagal, pakai fallback dari np.max(highs) candle yang
    sudah di-fetch di gate sebelumnya — zero wasted API calls.

    [A2] PENTING — Ini BUKAN All-Time High (ATH) sejati.
    Nilai ini adalah harga tertinggi dalam ~10 hari terakhir.
    Digunakan hanya sebagai referensi "ruang ke puncak observable terkini".
    """
    try:
        raw = gate_retry(
            ...
            currency_pair=pair,
            interval=GEM_TF,
            limit=GEM_HIGH90D_CANDLE_LIMIT,
        )
        if raw and len(raw) >= 96:   # minimal 1 hari data
...
def calc_vol30_score(
    ...
    """
    [UPG-C1] Bandingkan volume breakout terhadap baseline volume historis.
    Koin yang volumenya 50× di atas baseline lebih powerful dari koin yang
    8× di atas flat period yang kebetulan juga selalu sepi.
    ...
    [I1-NAME] v3.9: variabel di-rename agar akurat:
      vols_30d    → vols_buf   (window aktual = 10 hari, bukan 30)
      baseline_30 → baseline   (tidak ada angka 30 yang misleading)
      ratio_30d   → ratio      (konsisten)
    Window coverage: GEM_CANDLE_LIMIT=384 × 15m ≈ 10 hari.
    """
    try:
        # [E4-VOL30] prefetch selalu cukup (384 >= 150) — tidak ada separate fetch
        if prefetched_volumes is None or len(prefetched_volumes) < 150:
            return 0

        vols_buf = prefetched_volumes
        baseline = float(np.median(vols_buf[:-6]))   # median historis, kecuali 6 candle terakhir
        if baseline <= 0:
            return 0
        recent_max = float(np.max(vols_buf[-6:]))
        ratio      = recent_max / baseline
        if ratio >= 50:
            return 2
        if ratio >= 20:
            return 1
...
def format_summary(scanned: int, skipped: int, found: int, sent: int,
    ...
    if halted:
        return f"🛑 <b>GEM HUNTER v3.9 — HALT</b>\nBTC crash. Scan dilewati.\n<i>{now}</i>"
    return (
        f"💎 <b>GEM HUNTER v3.9 10/10 Final Scan Selesai</b>\n━━━━━━━━━━━━━━━━━━━━\n"
...
def run_scan() -> None:
    log("=" * 55)
    log(f"💎 GEM HUNTER v3.9 10/10 Final — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')}")
    log("=" * 55)
