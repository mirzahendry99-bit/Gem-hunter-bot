# indodax_pump_monitor_v56.py — v5.5.0
# Indodax Pump Signal Monitor | stack: Indodax API + pandas + Supabase + Telegram
...
# FASE 1 UNBLOCK: semua threshold dilonggarkan agar bot mulai detect sinyal.
# Noise diterima di fase ini — filter diperketat bertahap di Fase 3+.
PRICE_PUMP_PCT = float(os.environ.get("PRICE_PUMP_PCT", "0.5"))   # FASE1: 1.5→1.0→0.5
VOL_SPIKE_MULT = float(os.environ.get("VOL_SPIKE_MULT", "1.2"))   # FASE1: 2.0→1.5→1.2
RSI_OVERSOLD   = float(os.environ.get("RSI_OVERSOLD",   "35.0"))
RSI_RECOVERY   = float(os.environ.get("RSI_RECOVERY",   "50.0"))

# Volume filter — FASE 1: VOL_IDR_MIN diturunkan ke 500K agar microcap kecil masuk
VOL_IDR_MIN = float(os.environ.get("VOL_IDR_MIN", "500_000"))     # FASE1: 5M→1M→500K
...
# Anti fake pump ceiling — pump > nilai ini → bool=False (sudah top)
# FASE 4: 20% → 35% (pasar Indo sering pump 30%+ yang masih valid)
FAKE_PUMP_PCT = float(os.environ.get("FAKE_PUMP_PCT", "35.0"))
...
# FASE 1: MIN_SNAPS_SIGNAL diturunkan ke 2 agar bot tidak butuh terlalu banyak
# history sebelum bisa evaluate pair. Setelah akumulasi data cukup, naikkan ke 3+.
MIN_SNAPS_SIGNAL = int(os.environ.get("MIN_SNAPS_SIGNAL",  "2"))
MIN_SNAPS_RSI    = 16
...
# FASE 1: batas bawah diturunkan agar adaptive tidak stuck di nilai terlalu tinggi
# ketika WR rendah di awal (belum ada data). Bot tidak akan "auto-tighten" terlalu agresif.
PUMP_PCT_MIN, PUMP_PCT_MAX = 0.3, 6.0   # FASE1: 0.8→0.3
VOL_MULT_MIN,  VOL_MULT_MAX  = 1.0, 8.0   # FASE1: 1.2→1.0
# Bayesian prior (Jeffreys)
_BAYES_A, _BAYES_B = 0.5, 0.5

# ── Market Condition Filter (BTC trend guard)
BTC_TREND_ENABLED  = os.environ.get("BTC_TREND_ENABLED", "false").lower() == "true"  # FASE1: true→false
...
# ── FASE 5: Fine-tune constants
# Tier minimum untuk pengiriman sinyal ke Telegram.
# B = semua sinyal dengan score ≥ 1 dikirim (default — sama dengan Fase 4).
# A = hanya score ≥ 4; A+ = hanya score ≥ 7; S = hanya score ≥ 10.
# Gunakan A atau A+ jika channel terlalu ramai, atau S untuk sinyal premium saja.
TIER_MIN_SEND          = os.environ.get("TIER_MIN_SEND", "B").upper()
_TIER_SEND_THRESHOLD   = {"S": 10, "A+": 7, "A": 4, "B": 1}.get(TIER_MIN_SEND, 1)

# Baseline mode untuk gate_vol_spike.
# "median5" (default) = lebih stabil, warisan Fase 3 — tidak ada regresi false-spike.
# "min3"    = ultra-sensitif, tangkap spike lebih cepat, lebih banyak noise.
#             Aktifkan HANYA jika market sedang sideways dan bot sering miss early pump.
VOL_BASELINE_MODE      = os.environ.get("VOL_BASELINE_MODE", "median5").lower()

# Toggle trade pressure fetch (gate_trade_pressure + gate_ask_wall).
# Default: true — aktif karena memberi sinyal buy pressure berharga.
# Set false jika API Indodax sering rate-limit atau bot terlalu lambat.
ENABLE_TRADE_PRESSURE  = os.environ.get("ENABLE_TRADE_PRESSURE", "true").lower() == "true"

# FASE 4 OPTIMIZATION: SCORE_THRESHOLD dinaikkan ke 3.
# Threshold 1 (Fase 1) → noise terlalu banyak, pair dengan pump 0.5% + vol 1.2×
# yang kebetulan memenuhi G1 min akan lolos → sinyal berkualitas rendah.
# Threshold 3 memaksa bot butuh setidaknya G1 dan G2 yang keduanya bermakna,
# atau G1/G2 kuat + satu konfirmasi (RSI/breakout/sniper).
# Naikkan ke 4-5 jika masih terlalu banyak noise setelah tracking dimulai.
SCORE_THRESHOLD         = int(os.environ.get("SCORE_THRESHOLD",         "3"))
# FASE 4 OPTIMIZATION: Cooldown diaktifkan kembali ke 60 menit.
# Cooldown mencegah pair yang sama spam channel dalam satu sesi.
# Ini juga membantu distribusi sinyal: tidak ada 1 pair yang dominasi.
SIGNAL_COOLDOWN_MINUTES = int(os.environ.get("SIGNAL_COOLDOWN_MINUTES", "60"))

# ── Trading Hours Guard
# Bot hanya scan pada jam aktif WIB untuk menghindari false signal
# di jam sepi saat likuiditas tipis dan rentan manipulasi.
# Set SCAN_HOUR_START=0 dan SCAN_HOUR_END=24 untuk disable guard (24/7).
SCAN_HOUR_START = int(os.environ.get("SCAN_HOUR_START", "8"))   # jam mulai scan (WIB)
SCAN_HOUR_END   = int(os.environ.get("SCAN_HOUR_END",   "22"))  # jam berhenti scan (WIB)
SCAN_HOUR_GUARD = os.environ.get("SCAN_HOUR_GUARD", "true").lower() == "true"  # toggle

# ── Gate & Sniper Constants
# FASE 1 UNBLOCK: semua threshold dilonggarkan. Naikkan bertahap di Fase 3+.
BREAKOUT_PCT    = float(os.environ.get("BREAKOUT_PCT",    "0.5"))  # FASE1: 1.0→0.5
SNIPER_PUMP_PCT = float(os.environ.get("SNIPER_PUMP_PCT", "0.5"))  # FASE1: 0.8→0.5
SNIPER_VOL_MULT = float(os.environ.get("SNIPER_VOL_MULT", "1.2"))  # FASE1: 1.5→1.2

# ── Pre-Pump WATCH Constants
# [W1] Price sideways < WATCH_PRICE_FLAT_PCT% dalam 6 snapshot
# [W2] Vol_delta naik berturut-turut di 3 snapshot terakhir
# [W3] Vol_delta sekarang WATCH_VOL_BUILD_MIN× s/d WATCH_VOL_BUILD_MAX× baseline
WATCH_PRICE_FLAT_PCT  = float(os.environ.get("WATCH_PRICE_FLAT_PCT",  "3.0"))  # FASE1: 2.0→3.0
WATCH_VOL_BUILD_MIN   = float(os.environ.get("WATCH_VOL_BUILD_MIN",   "1.8"))  # floor >= WATCH_VOL_HARD_MIN
WATCH_VOL_BUILD_MAX   = float(os.environ.get("WATCH_VOL_BUILD_MAX",   "4.9"))
WATCH_MIN_SNAPS       = int(os.environ.get("WATCH_MIN_SNAPS",          "3"))   # FASE1: 5→3
MAX_WATCH_PER_RUN     = int(os.environ.get("MAX_WATCH_PER_RUN",        "5"))   # FASE1: 3→5
WATCH_VOL_HARD_MIN    = float(os.environ.get("WATCH_VOL_HARD_MIN",    "1.8"))  # FASE4: hardcoded→ENV

# ── WR Bucketing Constants
# Sinyal dengan WR bucket < MIN_BUCKET_WR di-skip (butuh MIN_BUCKET_SAMPLE sampel)
MIN_BUCKET_WR     = float(os.environ.get("MIN_BUCKET_WR",    "0.40"))
MIN_BUCKET_SAMPLE = int(os.environ.get("MIN_BUCKET_SAMPLE",  "5"))

# ── Trade Pressure Rate-Limit Guard
# Jeda antar per-pair API call (fetch_trades / fetch_depth) saat ENABLE_TRADE_PRESSURE=true.
# Default 50ms — cukup menghindari burst rate-limit Indodax tanpa memperlambat scan secara berarti.
# Naikkan ke 0.1–0.2 jika bot masih kena rate-limit. Set 0 untuk disable.
TRADE_PRESSURE_SLEEP_SEC = float(os.environ.get("TRADE_PRESSURE_SLEEP_SEC", "0.05"))
...
logging.basicConfig(
    level=logging.DEBUG,    # FASE 2: DEBUG aktif untuk visibility gate rejection
...
def build_snapshot(tickers: dict) -> dict:
    ...
    for pair, td in tickers.items():
        try:
            ...
            pass
    # FASE 2: log distribusi filter agar tahu berapa pair masuk snapshot
    log(
        f"build_snapshot: {len(data)} lolos | "
        f"{n_vol_low} vol<min | {n_vol_high} vol>max | "
        f"{n_price_zero} harga=0 | "
        f"total={len(tickers)}",
        "debug"
    )
...
def state_load_thresholds(state: dict) -> tuple[float, float, float, float]:
    ...
    Anti-spiral: jika saved threshold sudah > YAML baseline × 1.5 → reset ke baseline.
    ...
    # Anti-spiral guard
    # FASE 4 OPT-5: naikkan dari 1.3× → 1.5× agar adaptive tidak terlalu
    # cepat reset ke baseline saat WR rendah di awal. Dengan 1.3×,
    # threshold yang naik sedikit saja (misal 0.5% × 1.3 = 0.65%) sudah
    # di-reset. Dengan 1.5×, ada ruang adaptasi lebih luas sebelum reset.
    if pump > PRICE_PUMP_PCT * 1.5:
        log(f"📊 Anti-spiral reset: pump {pump:.2f}% → {PRICE_PUMP_PCT:.2f}%", "warn")
        pump = PRICE_PUMP_PCT
    if vol > VOL_SPIKE_MULT * 1.5:
...
# ══════════════════════════════════════════════════════════════════
#  FASE 5: COOLDOWN PER PAIR
# ══════════════════════════════════════════════════════════════════

def state_save_cooldown(state: dict, pair: str) -> dict:
    """
    Catat timestamp sinyal terakhir untuk pair ini.
    Dipanggil setiap kali sinyal berhasil dikirim ke Telegram.
    Format: state["cooldowns"] = {"btc_idr": 1723456789, ...}
    """
    state.setdefault("cooldowns", {})[pair] = int(time.time())
    return state


def state_is_on_cooldown(state: dict, pair: str) -> bool:
    """
    Return True jika pair masih dalam cooldown sejak sinyal terakhir.
    Tujuan: mencegah pair yang sama terus spam channel selama window cooldown.

    Cooldown mulai dari saat sinyal terakhir dikirim (bukan dari saat pair pertama muncul).
    Jika belum pernah kirim sinyal untuk pair ini → tidak cooldown (False).
    """
    last_ts = state.get("cooldowns", {}).get(pair, 0)
    if last_ts == 0:
        return False
    elapsed_minutes = (int(time.time()) - last_ts) / 60
    on_cooldown = elapsed_minutes < SIGNAL_COOLDOWN_MINUTES
    if on_cooldown:
        log(
            f"  ⏳ {pair} cooldown aktif: "
            f"{elapsed_minutes:.0f}/{SIGNAL_COOLDOWN_MINUTES} menit berlalu",
            "debug",
        )
    return on_cooldown
...
def gate_vol_confirm(history: list[dict], curr_vol: float) -> tuple[bool, int]:
    """
    FASE 4: AKTIF — volume delta acceleration.
    Deteksi apakah delta volume *meningkat* dalam VOL_CONFIRM_SNAPS snapshot terakhir.
    Ini berbeda dari gate_vol_spike: spike bisa sesaat, acceleration = trend.

    Return (True, ratio_int) jika delta curr >= 1.2× median delta recent.
    Return (False, 0) jika tidak cukup data atau acceleration tidak terkonfirmasi.
    """
    ...
    if len(deltas) < VOL_CONFIRM_SNAPS + 1:
        return False, 0
    curr_delta    = deltas[-1]
    recent_deltas = sorted([d for d in deltas[-VOL_CONFIRM_SNAPS - 1:-1] if d > 0])
    if not recent_deltas or curr_delta <= 0:
        return False, 0
    # Gunakan median recent deltas (lebih robust dari avg)
    n = len(recent_deltas)
    mid = n // 2
    baseline = recent_deltas[mid] if n % 2 != 0 else (recent_deltas[mid - 1] + recent_deltas[mid]) / 2
    if baseline <= 0:
        return False, 0
...
def gate_price_pump(curr: float, history: list[dict]) -> tuple[bool, float]:
    """
    Hitung pump_pct vs rata-rata 3 snapshot terakhir.
    Anti fake pump: pump > 20% = sudah top, bool=False (safety filter).
    pump_pct SELALU dikembalikan agar debug log bisa lihat nilai nyata,
    termasuk negatif (harga turun) dan extreme (> 20%).
    Di Fase 3, bool hanya dipakai untuk sniper_ok check — bukan blocker.
    """
    if not history:
        return False, 0.0
    ...
    # FASE 4: Anti fake pump ceiling naik 20% → FAKE_PUMP_PCT (default 35%).
    # Pasar Indo microcap sering pump 25-30% dalam satu window yang masih legit.
    # Nilai pct tetap dikembalikan untuk visibility log meski di atas ceiling.
    if pct > FAKE_PUMP_PCT:
...
def gate_vol_spike(curr_vol: float, history: list[dict]) -> tuple[bool, float]:
    """
    FASE 3 FIX: Baseline diganti median 5-snapshot → lebih stabil dari min().

    Masalah min(): jika ada 1 snapshot sepi di antara 3 terakhir,
    baseline = volume paling kecil → hampir semua pair "spike" relatif
    terhadap momen sepi tersebut → false positive noise.

    Median 5-snapshot: menggunakan titik tengah dari 5 snapshot terakhir
    sebagai "normal baseline". Lebih robust terhadap outlier (spike sesaat
    atau momen sepi sesaat). Spike nyata tetap terdeteksi karena harus
    secara signifikan di atas median, bukan hanya di atas momen terhening.

    FASE 5 F5-2: VOL_BASELINE_MODE menentukan strategi baseline.
      "median5" (default) — median dari 5 snapshot terakhir, stabil.
      "min3"               — min dari 3 snapshot terakhir, ultra-sensitif
                             untuk early spike detection. Lebih noisy.
    """
    if len(history) < 3:
        return False, 0.0
    ...
    if VOL_BASELINE_MODE == "min3":
        # F5-2: ultra-sensitive baseline — min dari 3 snap terakhir
        # Nangkep spike lebih cepat karena baseline = momen paling sepi.
        # Trade-off: false positive lebih tinggi jika ada 1 snapshot sepi.
        recent_vols = [h["vol"] for h in history[-3:] if h.get("vol", 0) > 0]
        if not recent_vols:
            return False, 0.0
        baseline = min(recent_vols)
    else:
        # Default: median5 (Fase 3, stabil)
        recent_vols = sorted([h["vol"] for h in history[-5:] if h.get("vol", 0) > 0])
        if not recent_vols:
            return False, 0.0
        n = len(recent_vols)
        baseline = recent_vols[n // 2] if n % 2 != 0 else (recent_vols[n // 2 - 1] + recent_vols[n // 2]) / 2

    if baseline <= 0:
        return False, 0.0
    ratio = curr_vol / baseline
...
def fetch_all_tickers() -> dict[str, dict]:
    """
    Ambil semua ticker Indodax (endpoint ticker_all).
    FIX v5.5: Tambah retry 3× dengan exponential backoff — konsisten dengan
    fetch_summaries(). Sebelumnya satu attempt gagal → seluruh scan abort.
    """
    for attempt in range(1, 4):
        try:
            ...
            log(f"fetch_all_tickers() attempt {attempt} failed: {e}", "warn")
            if attempt < 3:
                time.sleep(2 ** attempt)
    log("fetch_all_tickers() semua 3 attempt gagal — abort scan.", "error")
    return {}
...
def fetch_summaries() -> dict[str, dict]:
    ...
    FIX: Tambah retry 3× dengan exponential backoff (sama seperti tg()).
    Sebelumnya tidak ada retry — gagal sekali langsung return {} dan
    scoring pair kehilangan data historis 7 hari.
    """
    for attempt in range(1, 4):
        try:
            ...
            log(f"fetch_summaries() attempt {attempt} failed: {e}", "warn")
            if attempt < 3:
                time.sleep(2 ** attempt)
    log("fetch_summaries() semua 3 attempt gagal — lanjut tanpa data summaries.", "warn")
    return {}
...
def gate_trade_pressure(pair: str) -> tuple[bool, float, str]:
    """
    FASE 4: AKTIF — hanya dipanggil untuk kandidat yang sudah lolos score awal.
    Tidak menambah delay ke semua pair karena fetch hanya ke pair relevan.

    Return (True, ratio, summary) — passed selalu True (scoring only, bukan blocker).
    Jika API gagal atau data kosong, return (True, 0.0, reason) — tidak bunuh sinyal.
    """
    ...
    try:
        ...
        return True, ratio, summary
    except Exception as e:
        log(f"gate_trade_pressure({pair}): {e}", "debug")
...
# (WATCH_* constants ada di blok CONFIG di atas)
...
def detect_pre_pump(
    ...
    try:
        ...
        base_deltas = sorted([d for d in all_deltas[:-1] if d > 0])
        if not base_deltas or curr_delta <= 0:
            return None
        # FASE 3 FIX: Gunakan median bukan mean untuk baseline W3.
        # Mean bisa ditarik ke atas oleh satu spike besar di history,
        # sehingga vol_ratio terlihat kecil padahal build-up nyata terjadi.
        # Median lebih stabil dan membuat WATCH lebih konsisten.
        n_bd = len(base_deltas)
        baseline = base_deltas[n_bd // 2] if n_bd % 2 != 0 else (
            base_deltas[n_bd // 2 - 1] + base_deltas[n_bd // 2]) / 2
        if baseline <= 0:
            return None
        vol_ratio = curr_delta / baseline
        # FASE 4 FIX 4.4: vol_ratio minimum naik ke 1.8x (sebelumnya WATCH_VOL_BUILD_MIN=1.5).
        # vol_ratio 1.5–1.8x terlalu sering terjadi tanpa pre-pump nyata → spam WATCH.
        # Hard guard 1.8x di sini adalah filter awal sebelum cek WATCH_VOL_BUILD_MIN,
        # sehingga WATCH_VOL_BUILD_MIN env var masih bisa digunakan untuk tuning atas.
        if vol_ratio < WATCH_VOL_HARD_MIN:
            return None   # WATCH_VOL_HARD_MIN: build-up belum signifikan (tunable via ENV)
...
def check_early_signals(
    ...
    for w in active_watches:
        ...
        # FIX: Gunakan window tetap +2% dari EARLY_BREAKOUT_PCT, bukan PRICE_PUMP_PCT × 2.
        # Sebelumnya: PRICE_PUMP_PCT=0.5% → upper bound hanya 1.0% → early signal
        # terpicu di zona yang nyaris sama dengan pump signal, SL 3% langsung kena.
        # Sekarang: window fixed 2% dari threshold, tidak bergantung pada PRICE_PUMP_PCT.
        if EARLY_BREAKOUT_PCT <= pct_change < EARLY_BREAKOUT_PCT + 2.0:
...
def analyze_pair(
    ...
    btc_label:   str = "neutral",   # F5-1: market context scoring
) -> Optional[dict]:
    ...
    try:
        ...
        # ── FASE 2: DEBUG VISIBILITY LOG ─────────────────────────────
        log(
            f"{pair} | pump={pump_pct:.2f}%({'OK' if pump_ok else 'FAIL'}) | "
            f"vol={vol_ratio:.2f}x({'OK' if vol_ok else 'FAIL'}) | "
            f"breakout={'OK' if break_ok else 'FAIL'} | "
            f"sniper={'OK' if sniper_ok else 'FAIL'} | "
            f"extreme={'YES' if is_extreme else 'NO'} | "
            f"snaps={len(history)}",
            "debug"
        )
        # ─────────────────────────────────────────────────────────────

        # ══ FASE 3: SCORING MODEL — murni proporsional, zero cliff-edge ═
        # Setiap kondisi berkontribusi ke skor.  Tidak ada yang memblokir.
        # Pump kecil + vol normal = score rendah tapi tetap lolos (threshold=1).
        # Pump besar + vol meledak = score tinggi → tier S/A+.
        sig_score = 0
        _score_parts: list[str] = []  # Fase 2 visibility — tetap aktif

        # G1 — Pump strength (0-5): granular, tidak cliff-edge
        # FASE 4 FIX 4.1: pump 0.5–1.0% tidak lagi dapat skor.
        # Range ini terlalu lemah untuk dikonfirmasi sebagai sinyal pump nyata
        # — sering noise market biasa. Threshold efektif minimum naik ke 1.0%.
        if   pump_pct >= 8.0: g1 = 5
        elif pump_pct >= 5.0: g1 = 4
        elif pump_pct >= 2.0: g1 = 3
        elif pump_pct >= 1.0: g1 = 2
        else:                 g1 = 0   # 4.1: 0.5–1.0% = 0 (sebelumnya g1=1)
        sig_score += g1
        _score_parts.append(f"G1+{g1}(pump={pump_pct:.1f}%)")

        # G2 — Volume spike (0-5): granular
        # FASE 4 FIX 4.1: vol 1.2–1.5x tidak lagi dapat skor.
        # Volume naik 20–50% dari median adalah noise normal — bukan spike bermakna.
        # Threshold efektif minimum naik ke 1.5x (g2 mulai dari sini).
        if   vol_ratio >= 7.0: g2 = 5
        elif vol_ratio >= 4.0: g2 = 4
        elif vol_ratio >= 2.0: g2 = 3
        elif vol_ratio >= 1.5: g2 = 2
        else:                  g2 = 0   # 4.1: 1.2–1.5x = 0 (sebelumnya g2=1)
        sig_score += g2
        _score_parts.append(f"G2+{g2}(vol={vol_ratio:.1f}x)")

        # G3 — Breakout bonus (+2): konfirmasi strength, bukan prasyarat
        if break_ok:
            sig_score += 2
            _score_parts.append("G3+2(breakout)")
        else:
            _score_parts.append("G3+0(no_breakout)")

        # Sniper path bonus (+2 FASE5): early entry sebelum breakout terkonfirmasi.
        # Naik dari +1 → +2 karena entry lebih awal secara konsisten menghasilkan
        # R/R lebih baik (entry lebih rendah, SL lebih ketat, ruang ke TP lebih besar).
        if sniper_ok:
            sig_score += 2
            _score_parts.append("sniper+2")

        # RSI bonus (+1) — tidak memblokir, hanya konfirmasi momentum
        _, curr_rsi, has_rsi = gate_rsi_cross(curr_price, history)
        if not is_extreme and has_rsi and curr_rsi >= 45:
            sig_score += 1
            _score_parts.append(f"rsi+1({curr_rsi:.0f})")

        # Volume acceleration bonus — bypass di Fase 1/2/3, uncomment Fase 4
        vol_acc_ok, vol_acc_ratio = gate_vol_confirm(history, vol_idr)
        if vol_acc_ok:
            sig_score += 1
            _score_parts.append(f"vol_acc+1({vol_acc_ratio}x)")
            log(f"{pair} vol_acc bonus +1 (ratio={vol_acc_ratio}x)", "debug")

        # ── F5-1: BTC Market Context — scoring adaptif (bukan blocker)
        # BTC bearish = kurangi 1 poin (market headwind), bullish = tambah 1 poin.
        # Neutral tidak berpengaruh. Ringan sengaja — BTC bukan oracle sempurna
        # untuk microcap Indodax yang sering pump independent dari BTC.
        if btc_label == "bearish":
            sig_score = max(0, sig_score - 1)
            _score_parts.append("btc_bearish-1")
            log(f"{pair} btc_bearish penalty -1", "debug")
        elif btc_label == "bullish":
            sig_score += 1
            _score_parts.append("btc_bullish+1")
            log(f"{pair} btc_bullish bonus +1", "debug")

        # Buy pressure bonus — hanya fetch jika score awal >= 3 (Fix 4.3).
        # Sebelumnya: fetch jika score > 0 → terlalu banyak API call ke pair lemah.
        # Sekarang: threshold 3 memastikan G1+G2 sudah bermakna sebelum fetch trades.
        # FASE 5 F5-4: ENABLE_TRADE_PRESSURE=false → skip fetch sepenuhnya.
        buyer_ratio = 0.0
        _bp_summary = "skip(score<3)"
        if sig_score >= 3 and ENABLE_TRADE_PRESSURE:   # 4.3: naik dari >0 ke >=3
            try:
                _, buyer_ratio, _bp_summary = gate_trade_pressure(pair)
                if   buyer_ratio >= 70: bp = 3
                elif buyer_ratio >= 60: bp = 2
                elif buyer_ratio >= 50: bp = 1
                else:                   bp = 0
                bp = min(bp, 2)   # Fix 4.2: cap buy pressure ke +2 (sebelumnya maks +3)
                if bp > 0:
                    sig_score += bp
                    _score_parts.append(f"buy_pressure+{bp}({buyer_ratio:.0f}%)")
                else:
                    _score_parts.append(f"buy_pressure+0({buyer_ratio:.0f}%)")
                log(f"{pair} trade_pressure: {_bp_summary}", "debug")
            except Exception as e:
                log(f"{pair} trade_pressure err: {e}", "debug")
        elif sig_score >= 3:
            # F5-4: trade pressure dinonaktifkan via ENABLE_TRADE_PRESSURE=false
            _bp_summary = "disabled(env)"
            _score_parts.append("buy_pressure+0(disabled)")

        # Ask wall — info saja, no penalty (juga skip jika trade pressure dinonaktifkan)
        wall_ratio = 0.0
        if ENABLE_TRADE_PRESSURE:
            try:
                ...
                pass
            # FIX v5.5: rate-limit guard — jeda antar per-pair API call
            # mencegah burst request ke Indodax saat scan ratusan pair.
            if TRADE_PRESSURE_SLEEP_SEC > 0:
                time.sleep(TRADE_PRESSURE_SLEEP_SEC)
        ...
        # ── Trigger: score >= threshold → signal
        # FASE 1 UNBLOCK: Gunakan module-level SCORE_THRESHOLD (default sekarang = 1).
        # KRITIS: sebelumnya ada local re-read env "SCORE_THRESHOLD" di sini yang
        # membuat perubahan default di module level tidak berpengaruh ke analyze_pair.
        # Diperbaiki: module-level SCORE_THRESHOLD kini menjadi single source of truth.
        # FASE 2 VISIBILITY: jika score > 0 tapi gagal threshold → log di INFO (bukan DEBUG)
        # agar "near miss" pair terlihat tanpa perlu log level DEBUG penuh.
        if sig_score < SCORE_THRESHOLD:
            _near_miss = sig_score > 0
            log(
                f"{pair} SCORE_FAIL {sig_score}<{SCORE_THRESHOLD} | "
                + " | ".join(_score_parts),
                "info" if _near_miss else "debug",   # FASE 2: near-miss → INFO
            )
            _reject(f"SCORE_fail_{sig_score}")
            return None

        log(
            f"{pair} SCORE_PASS {sig_score}>={SCORE_THRESHOLD} | "
            + " | ".join(_score_parts),
            "debug"
        )
        # BONUS v5.5: SCORE_DIST log — mudah di-grep untuk analisis distribusi
        log(f"SCORE_DIST | pair={pair} | score={sig_score} | parts={','.join(_score_parts)}", "debug")
...
_TIER_LABELS = {
    ...
    "A+": ("🏆", "TIER A+",  7),
    "A":  ("🥇", "TIER A",   4),
    "B":  ("📡", "TIER B",   1),   # FIX v5.5: tier B punya label sendiri, bukan fallback ke A
}


def calc_score(sig: dict) -> tuple[int, str]:
    """
    Fase 3: calc_score langsung pakai sig_score dari analyze_pair.
    sig_score sudah granular (0-13+) dan tidak perlu dihitung ulang.
    Hanya tambah history reliability yang belum ada di sig_score.

    Tier (disesuaikan dengan rentang sig_score baru):
      S    >= 10  — pump kuat + vol meledak + konfirmasi
      A+   >=  7  — sinyal solid, 2+ komponen kuat
      A    >=  4  — sinyal layak, 1 komponen kuat + 1 konfirmasi
      B    >=  1  — sinyal lemah, pantau & hati-hati
      SKIP  = 0   — tidak ada komponen sama sekali
    """
    # Ambil sig_score dari analyze_pair (sudah granular G1/G2/G3/sniper/rsi/dll)
    score = sig.get("sig_score", 0)

    # History reliability — tambahan di sini, belum dihitung di sig_score
    s = sig.get("snaps", 0)
    if   s >= 16: score += 2
    elif s >= 8:  score += 1
    ...
    elif score >= 1:  tier = "B"     # Fase 3: B tier tetap dikirim
    else:             tier = "SKIP"  # score 0: benar-benar tidak ada sinyal
    return score, tier
...
_SB_CLIENT = None  # Fase 3: singleton Supabase — dibuat sekali, dipakai ulang

def _get_sb():
    """
    Return Supabase client singleton, atau None jika tidak tersedia/konfigurasi.

    FASE 3 FIX: Sebelumnya _get_sb() memanggil _sb_create() setiap kali dipanggil
    → buat koneksi baru tiap DB call dalam satu run → boros dan memperlambat scan.
    Singleton pattern: client dibuat sekali saat pertama dipanggil, lalu di-reuse.
    Aman untuk single-threaded bot seperti ini.
    """
    global _SB_CLIENT
...
def db_equity_report() -> Optional[str]:
    ...
    return (
        f"📊 <b>Equity Report</b> <i>(Simulasi — bukan uang nyata)</i>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
...
def db_load_winrate() -> dict:
    ...
    FASE 4: Tambah WR per mode — SNIPER / NORMAL / EARLY.
    Return {
      ...
      "early_wr": float,
      "n_early": int,
      "mode_wr": {           # FASE 4: breakdown per mode
          "SNIPER": {"wr": float, "n": int, "n_win": int},
          "NORMAL": {"wr": float, "n": int, "n_win": int},
          "EARLY":  {"wr": float, "n": int, "n_win": int},
      }
    }
    ...
    try:
        rows = (
            sb.table(DB_TABLE)
            .select("result, pnl_pct, pump_pct, mode")
            .not_.is_("result", "null")
            .execute()
            .data
        ) or []
    ...
    # ── FASE 4: WR per mode (SNIPER / NORMAL / EARLY dari kolom mode)
    mode_wr: dict[str, dict] = {}
    for mode_name in ("SNIPER", "NORMAL", "EARLY"):
        mode_rows = [r for r in rows if (r.get("mode") or "").upper() == mode_name]
        n_m       = len(mode_rows)
        n_m_win   = sum(1 for r in mode_rows if (r.get("result") or "").upper() in WIN_SET)
        if n_m > 0:
            m_wr = round((n_m_win + _BAYES_A) / (n_m + _BAYES_A + _BAYES_B), 3)
            mode_wr[mode_name] = {"wr": m_wr, "n": n_m, "n_win": n_m_win}
            icon = "🟢" if m_wr >= 0.55 else ("🟡" if m_wr >= 0.45 else "🔴")
            log(f"📊 {mode_name:6s} WR: {icon} {m_wr:.0%} ({n_m_win}/{n_m})")
        else:
            mode_wr[mode_name] = {"wr": 0.5, "n": 0, "n_win": 0}
...
# (MIN_BUCKET_WR & MIN_BUCKET_SAMPLE ada di blok CONFIG di atas)
...
def adapt_thresholds(
    ...
    # FASE 4: minimum 50 sampel closed sebelum adaptive threshold aktif
    # (lebih aman dari sebelumnya yang hanya butuh WR_MIN_SAMPLE=15)
    ADAPT_MIN_SAMPLE = 50
    if not wr_data or wr_data.get("n_total", 0) < ADAPT_MIN_SAMPLE:
        n = wr_data.get("n_total", 0) if wr_data else 0
        log(f"📊 Adaptive: belum cukup sampel ({n}/{ADAPT_MIN_SAMPLE}) — threshold tidak berubah")
    ...
    # ── FASE 4: max perubahan ±10% per run (lebih konservatif dari sebelumnya)
    # Sebelumnya: perketat +10% atau longgar -10% (sama, tidak berubah)
    # Sekarang: tetap ±10% tapi hanya aktif saat WR jelas di luar zona
    if wr < 0.40:
        factor = 1.05   # FASE4: hanya +5% (lebih pelan, hindari over-tightening)
        log(f"📊 Adaptive: WR={wr:.0%} < 40% → PERKETAT ×{factor} "
            f"(cap: pump≤{max_pump}% vol≤{max_vol}×)")
    elif wr > 0.65:
        factor = 0.95   # FASE4: hanya -5% per run
...
def format_signal(sig: dict, wr_data: dict | None = None) -> str:
    ...
    tier_emoji, tier_label, _ = _TIER_LABELS.get(tier, ("📡", "TIER B", 1))  # FIX v5.5: fallback ke B, bukan A
    ...
    if wr_data and wr_data.get("n_total", 0) >= WR_MIN_SAMPLE:
        ...
        # FASE 4: mode WR breakdown — tampilkan jika data ada
        mode_wr  = wr_data.get("mode_wr", {})
        mode_lines = []
        for m_name in ("SNIPER", "NORMAL", "EARLY"):
            m = mode_wr.get(m_name, {})
            if m.get("n", 0) > 0:
                m_icon = "🟢" if m["wr"] >= 0.55 else ("🟡" if m["wr"] >= 0.45 else "🔴")
                mode_lines.append(f"{m_name}:{m_icon}{m['wr']:.0%}({m['n_win']}/{m['n']})")
        mode_wr_line = ("  <i>" + " | ".join(mode_lines) + "</i>\n") if mode_lines else ""
        hist_wr += mode_wr_line
    else:
        n = wr_data.get("n_total", 0) if wr_data else 0
...
def run_scan() -> None:
    ...
    # ── TRADING HOURS GUARD: hanya scan pada jam aktif WIB
    # Menghindari false signal di jam sepi (tengah malam) saat likuiditas tipis.
    # Disable dengan: SCAN_HOUR_GUARD=false atau SCAN_HOUR_START=0 SCAN_HOUR_END=24.
    if SCAN_HOUR_GUARD and not (SCAN_HOUR_START <= SCAN_HOUR_END):
        pass  # misconfigured range → tetap scan (fail-open)
    elif SCAN_HOUR_GUARD:
        _now_hour = datetime.now(WIB).hour
        if not (SCAN_HOUR_START <= _now_hour < SCAN_HOUR_END):
            _msg_off = (
                f"🌙 <b>Di Luar Jam Scan</b>\n"
                f"Jam sekarang: <b>{_now_hour:02d}:xx WIB</b>\n"
                f"Jam aktif scan: <b>{SCAN_HOUR_START:02d}:00 – {SCAN_HOUR_END:02d}:00 WIB</b>\n"
                f"<i>Scan ditahan untuk menghindari false signal di jam sepi.</i>\n"
                f"⏰ {datetime.now(WIB).strftime('%d/%m/%Y %H:%M WIB')}"
            )
            log(f"🌙 Di luar jam scan ({_now_hour}:xx WIB — aktif {SCAN_HOUR_START}–{SCAN_HOUR_END}). Skip.", "warn")
            tg(_msg_off)
            return

    log("=" * 64)
    log(f"🔍 INDODAX PUMP SIGNAL MONITOR v{__version__} (FASE 5 — TIER LOCK & LIFECYCLE)")
    log(f"  Pump≥{PRICE_PUMP_PCT}% | Vol≥{VOL_SPIKE_MULT}× ({VOL_BASELINE_MODE}) | VolMin={_fmt_idr(VOL_IDR_MIN)} | RSI {RSI_OVERSOLD:.0f}→{RSI_RECOVERY:.0f}")
    log(f"  SCORE_THRESHOLD={SCORE_THRESHOLD} | TIER_MIN={TIER_MIN_SEND}(>={_TIER_SEND_THRESHOLD}) | COOLDOWN={SIGNAL_COOLDOWN_MINUTES}m | SNAPS_MIN={MIN_SNAPS_SIGNAL}")
    log(f"  Sniper≥{SNIPER_PUMP_PCT}%+{SNIPER_VOL_MULT}× | Breakout≥{BREAKOUT_PCT}% | MaxSig={MAX_SIGNALS_PER_RUN} | MaxWatch={MAX_WATCH_PER_RUN}")
    log(f"  TradePressure={'ON' if ENABLE_TRADE_PRESSURE else 'OFF'} | BucketFilter: MIN_WR={MIN_BUCKET_WR:.0%} | AntiSpiral: 1.5×")
    log("=" * 64)
    ...
    # ── SUPABASE CONNECTIVITY CHECK — alert jika DB tidak terhubung
    # Fitur yang non-aktif tanpa DB: WR tracking, outcome evaluation,
    # equity report, WR bucketing, blacklist. Bot tetap scan tapi blind.
    if not _get_sb():
        _sb_reason = "library tidak tersedia" if not _SB_AVAILABLE else "SUPABASE_URL/KEY tidak di-set"
        log(f"⚠️  Supabase tidak terhubung ({_sb_reason}) — WR/outcome tracking OFF.", "warn")
        tg(
            f"⚠️ <b>Supabase Tidak Terhubung</b>\n"
            f"Alasan: <code>{_sb_reason}</code>\n"
            f"Fitur non-aktif: WR tracking, equity report, outcome eval, blacklist.\n"
            f"<i>Bot tetap scan tapi tidak belajar dari histori sinyal.</i>\n"
            f"⏰ {datetime.now(WIB).strftime('%d/%m/%Y %H:%M WIB')}"
        )
    else:
        log("✅ Supabase terhubung.")
    ...
    # FASE 2 VISIBILITY: distribusi score semua pair yang melewati PRE_FILTER.
    # Membantu identifikasi bottleneck: apakah masalah di pump, vol, atau threshold.
    score_dist: dict[str, int] = {"score_0": 0, "score_1-2": 0, "score_3-4": 0, "score_5+": 0}

    # FASE 5 F5-3: Lifecycle lock — set pair yang sedang dalam WATCH lifecycle.
    # Pair yang ada di WATCH list tidak boleh masuk jalur sinyal NORMAL di run yang sama.
    # Mereka akan diproses oleh check_early_signals() → jalur EARLY.
    # Ini mencegah overlap lifecycle: pair di WATCH tidak dikirim dua kali (WATCH + NORMAL).
    active_watch_pairs = {w["pair"] for w in state_get_active_watches(state)}
    # Set pair yang sudah punya open trade (dari portfolio) — skip di level scan, bukan hanya dispatch.
    # Ini memastikan pair open tidak membuang slot analyze_pair dan tidak masuk candidates.
    portfolio_pre = db_portfolio_status()
    open_pairs_pre = portfolio_pre.get("open_pairs", set())

    for pair, ticker_data in tickers.items():
        scanned += 1
        history = get_pair_history(state, pair)

        # ── F5-3: Lifecycle lock — pair di WATCH → jalur EARLY, skip NORMAL
        if pair in active_watch_pairs:
            gate_stats["WATCH_LIFECYCLE"] = gate_stats.get("WATCH_LIFECYCLE", 0) + 1
            log(f"  ⚡ {pair:20s} WATCH_LIFECYCLE — diproses di early_signals", "debug")
            continue   # akan diproses oleh check_early_signals()

        # ── F5-3: Open trade skip — jangan masuk candidates jika sudah open
        if pair in open_pairs_pre:
            gate_stats["OPEN_TRADE_SKIP"] = gate_stats.get("OPEN_TRADE_SKIP", 0) + 1
            log(f"  ⛔ {pair:20s} OPEN_TRADE_SKIP — sudah ada posisi aktif", "debug")
            continue

        # ── Main pump signal (pass state untuk blacklist + summaries untuk gate 5-7)
        sig = analyze_pair(pair, ticker_data, history, gate_stats, state, summaries, btc_label)

        # FASE 3 FIX: score_dist harus mencerminkan SEMUA pair yang masuk scan,
        # bukan hanya yang lolos analyze_pair. Pair yang ditolak analyze_pair
        # (karena score < threshold) juga punya sig_score di gate_stats —
        # kita track langsung dari gate_stats["SCORE_fail_N"].
        # Untuk pair yang lolos, ambil dari sig["sig_score"].
        if sig:
            _raw_score = sig.get("sig_score", 0)
            if   _raw_score == 0:    score_dist["score_0"]   += 1
            elif _raw_score <= 2:    score_dist["score_1-2"] += 1
            elif _raw_score <= 4:    score_dist["score_3-4"] += 1
            else:                    score_dist["score_5+"]  += 1

            # FASE 5: Cooldown per pair — skip jika pair ini sudah kirim sinyal
            # dalam SIGNAL_COOLDOWN_MINUTES menit terakhir
            if state_is_on_cooldown(state, pair):
                gate_stats["COOLDOWN"] = gate_stats.get("COOLDOWN", 0) + 1
                log(f"  ⏳ {pair:20s} COOLDOWN — skip", "debug")
                continue

            score, tier = calc_score(sig)

            # ── FASE 5: WR-based score multiplier per mode ──────────────────
            # Sistem belajar dari hasil real: WR tinggi → boost skor, rendah → penalti.
            # Gunakan multiplier (bukan +/-1) agar efek proporsional terhadap skor awal.
            # SNIPER WR bagus → ×1.2; EARLY WR bagus → ×1.1; buruk → ×0.9.
            # Hanya aktif jika mode tersebut punya minimal 5 data closed.
            if wr_data:
                mode_wr_data = wr_data.get("mode_wr", {})
                if sig.get("is_sniper"):
                    _sig_mode = "SNIPER"
                elif sig.get("pump_pct", 0) < PRICE_PUMP_PCT:
                    _sig_mode = "EARLY"
                    ...
                    _sig_mode = "NORMAL"
                _m_info = mode_wr_data.get(_sig_mode, {})
                if _m_info.get("n", 0) >= 5:
                    _m_wr = _m_info.get("wr", 0.5)
                    if _m_wr >= 0.65:
                        # Mode ini punya WR bagus — boost proporsional
                        _mult = 1.2 if _sig_mode == "SNIPER" else 1.1
                        _old  = sig.get("sig_score", 0)
                        sig["sig_score"] = min(15, round(_old * _mult))
                        log(
                            f"  📈 {pair:20s} WR boost ×{_mult} "
                            f"({_sig_mode} WR={_m_wr:.0%}) "
                            f"{_old}→{sig['sig_score']}",
                            "debug",
                        )
                    elif _m_wr < 0.40:
                        # Mode ini sering kalah — penalti proporsional
                        _old  = sig.get("sig_score", 0)
                        sig["sig_score"] = max(0, round(_old * 0.9))
                        log(
                            f"  📉 {pair:20s} WR penalty ×0.9 "
                            f"({_sig_mode} WR={_m_wr:.0%}) "
                            f"{_old}→{sig['sig_score']}",
                            "debug",
                        )
            ...
            # FASE 4 OPT-3: Hapus redundant threshold check di sini.
            # analyze_pair() sudah memfilter dengan SCORE_THRESHOLD (baris ~1397).
            # Pair yang sampai di sini dijamin score >= SCORE_THRESHOLD.
            # Check kedua hanya diperlukan jika ada WR penalty yang bisa
            # menurunkan score di bawah threshold — retain untuk keamanan.
            if score < SCORE_THRESHOLD:
                gate_stats["SCORE_BELOW_THRESHOLD"] = gate_stats.get("SCORE_BELOW_THRESHOLD", 0) + 1
                log(
                    f"  ⚠ {pair:20s} SKIP score={score} < threshold={SCORE_THRESHOLD} "
                    f"(setelah WR penalty)",
                    "info",
                )
                continue

            # FASE 5 F5-1: Tier lock — sinyal dengan tier terlalu rendah di-skip.
            # TIER_MIN_SEND mengontrol berapa tier minimum yang dikirim ke channel.
            # Default "B" = semua tier dikirim (score ≥ 1 cukup).
            # Set ke "A" untuk quiet mode: hanya tier A/A+/S yang masuk.
            # Ini berbeda dari SCORE_THRESHOLD: threshold memfilter di analyze_pair,
            # tier lock memfilter di sini — setelah WR adjustment.
            if score < _TIER_SEND_THRESHOLD:
                gate_stats["TIER_LOCK"] = gate_stats.get("TIER_LOCK", 0) + 1
                log(
                    f"  ⚠ {pair:20s} SKIP tier={tier} score={score} "
                    f"< TIER_MIN_SEND={TIER_MIN_SEND}(>={_TIER_SEND_THRESHOLD})",
                    "info",
                )
                continue

            # WR bucket — dihitung untuk info, filter, & logging
            pred_wr, bucket_key, has_bucket = predict_wr(sig, wr_buckets)
            sig["pred_wr"]    = pred_wr
            sig["bucket_key"] = bucket_key
            sig["has_bucket"] = has_bucket

            # FASE 4 OPT-4: BUCKET_LOW_WR filter diaktifkan jika bucket sudah terisi.
            # Bucket terisi = ada ≥ MIN_BUCKET_SAMPLE sinyal closed di bucket ini.
            # Jika belum ada data → sinyal tetap lolos (has_bucket=False).
            if has_bucket and pred_wr < MIN_BUCKET_WR:
                gate_stats["BUCKET_LOW_WR"] = gate_stats.get("BUCKET_LOW_WR", 0) + 1
                log(
                    f"  ⚠ {pair:20s} SKIP bucket WR={pred_wr:.0%} < {MIN_BUCKET_WR:.0%} "
                    f"[{bucket_key}]",
                    "info",
                )
                continue
        ...
        # ── Pre-pump watch (hanya jika tidak ada pump signal)
        else:
    ...
    # Gate breakdown — FASE 2: top 10, tambah persen lolos
    log("─" * 56)
    log(f"📊 GATE REJECTION BREAKDOWN (scanned={scanned}):")
    if gate_stats:
        total_rejected = sum(gate_stats.values())
        for reason, count in sorted(gate_stats.items(), key=lambda x: -x[1])[:10]:
            pct = count / scanned * 100 if scanned else 0
            log(f"  {reason:<38} {count:>4}× ({pct:.0f}% pair)")
        log(f"  {'TOTAL REJECTED':<38} {total_rejected:>4}×")
        pass_count = scanned - total_rejected
        pass_pct   = pass_count / scanned * 100 if scanned else 0
        log(f"  {'LOLOS (kandidat signal)':<38} {len(candidates):>4}× ({pass_pct:.1f}% pass rate aprox.)")
    else:
        log("  (tidak ada rejection tercatat)")

    # ── FASE 3 FIX: Score Distribution Histogram (diperluas) ────────────
    # Selain pair yang lolos analyze_pair, tambahkan pair yang ditolak
    # karena SCORE_fail dari gate_stats agar histogram representatif.
    # SCORE_fail_0 = pair masuk PRE_FILTER tapi score=0 → score_0
    # SCORE_fail_1 = score=1 tapi di bawah threshold → score_1-2
    # SCORE_fail_2 = score=2 tapi di bawah threshold → score_1-2
    for key, cnt in gate_stats.items():
        if key.startswith("SCORE_fail_"):
            try:
                fail_score = int(key.split("_")[-1])
                if fail_score == 0:
                    score_dist["score_0"]   += cnt
                elif fail_score <= 2:
                    score_dist["score_1-2"] += cnt
                elif fail_score <= 4:
                    score_dist["score_3-4"] += cnt
                else:
                    score_dist["score_5+"]  += cnt
            except ValueError:
                pass
    ...
    if total_scored > 0:
        log("─" * 56)
        log(f"📈 SCORE DISTRIBUTION ({total_scored} pair masuk scoring):")
        bars = {"score_0": "░", "score_1-2": "▒", "score_3-4": "▓", "score_5+": "█"}
        for band, cnt in score_dist.items():
            pct = cnt / total_scored * 100
            bar = bars[band] * min(int(pct / 5), 20)
            log(f"  {band:<10} {cnt:>4}× ({pct:>5.1f}%) {bar}")

        # ── FASE 2: Bottleneck Identifier ────────────────────────────────
        top_gate, top_count = max(gate_stats.items(), key=lambda x: x[1]) if gate_stats else ("N/A", 0)
        top_pct = top_count / scanned * 100 if scanned else 0
        log("─" * 56)
        log(f"🔍 BOTTLENECK: [{top_gate}] memblokir {top_count}× ({top_pct:.0f}% pair)")
        if "vol_out_of_range" in top_gate:
            log("   → Solusi: turunkan VOL_IDR_MIN atau naikkan VOL_IDR_MAX")
        elif "more_snaps" in top_gate:
            log("   → Solusi: tunggu lebih banyak run, atau turunkan MIN_SNAPS_SIGNAL")
        elif "SCORE_fail_0" in top_gate:
            log("   → Solusi: turunkan PRICE_PUMP_PCT atau VOL_SPIKE_MULT — pair tidak ada movement")
        elif "SCORE_fail" in top_gate:
            log(f"   → Solusi: turunkan SCORE_THRESHOLD (sekarang={SCORE_THRESHOLD})")
        elif "SCORE_BELOW" in top_gate:
            log(f"   → Info: sinyal ditolak setelah WR adjustment (threshold={SCORE_THRESHOLD})")
        elif "BUCKET_LOW_WR" in top_gate:
            log(f"   → Info: bucket WR terlalu rendah (MIN_BUCKET_WR={MIN_BUCKET_WR:.0%})")
        elif "BLACKLISTED" in top_gate:
            log("   → Info: banyak pair di blacklist, cek DB untuk SL rate tinggi")
        elif "COOLDOWN" in top_gate:
            log(f"   → Solusi: turunkan SIGNAL_COOLDOWN_MINUTES (sekarang={SIGNAL_COOLDOWN_MINUTES}m)")
        # ──────────────────────────────────────────────────────────────────
    log("─" * 56)
    ...
    # Kirim sinyal (sorted: sig_score desc, pump_pct desc, vol_ratio desc)
    candidates.sort(key=lambda x: (-x.get("sig_score", 0), -x["pump_pct"], -x["vol_ratio"]))
    sent      = 0
    skipped   = 0
    # F5-3: Gunakan portfolio_pre yang sudah di-fetch sebelum scan loop.
    # open_pairs_pre sudah dipakai di scan loop untuk lifecycle lock,
    # sehingga candidates dijamin tidak mengandung pair open trade.
    # Re-fetch tidak diperlukan — portfolio status tidak berubah selama scan.
    ...
    for sig in candidates[:MAX_SIGNALS_PER_RUN]:
        ...
        # Ini safety net — lifecycle lock di scan loop seharusnya sudah mencegah ini.
        ...
        if tg(format_signal(sig, wr_data)):
            ...
            state = state_save_cooldown(state, pair)   # FASE 5: catat cooldown
    ...
    if early_signals:
        ...
        for es in early_signals[:MAX_WATCH_PER_RUN]:
            # BUGFIX FASE4: gunakan es["pair"], bukan 'pair' dari loop scan sebelumnya
            if es["pair"] in portfolio.get("open_pairs", set()):
            ...
            if tg(format_early_signal(es)):
                ...
                state = state_save_cooldown(state, es["pair"])   # FASE 5
                portfolio["open"]       += 1
    ...
    if wr_data and wr_data.get("n_total", 0) >= WR_MIN_SAMPLE:
        ...
        # FASE 4: tampilkan WR per mode di summary
        mode_wr = wr_data.get("mode_wr", {})
        mode_parts = []
        for m_name in ("SNIPER", "NORMAL", "EARLY"):
            m = mode_wr.get(m_name, {})
            if m.get("n", 0) > 0:
                icon = "🟢" if m["wr"] >= 0.55 else ("🟡" if m["wr"] >= 0.45 else "🔴")
                mode_parts.append(f"{m_name}:{icon}{m['wr']:.0%}({m['n']})")
        if mode_parts:
            wr_line += "\n         " + " | ".join(mode_parts)
...
class TestGatePricePump:
    ...
    def test_no_pump_flat(self):
        ...
        # FASE 4: PRICE_PUMP_PCT default = 1.0, jadi +1% (pct=1.0) tepat di batas.
        # gate_price_pump return ok jika pct >= PRICE_PUMP_PCT, jadi ini bisa True atau False
        # tergantung env. Yang penting: return type benar.
        assert isinstance(ok, bool)
        assert abs(pct - 1.0) < 0.01   # harga naik 1% dari flat history
...
class TestGateVolSpike:
    ...
    def test_spike_detected(self):
        """vol sekarang >= VOL_SPIKE_MULT × rata-rata vol historis."""
        base = 50_000_000.0
        # History: semua snapshot vol = base (stabil)
        history = [{"ts": i, "last": 1000.0, "vol": base} for i in range(10)]
        # Spike: vol sekarang = base × VOL_SPIKE_MULT × 2 (jelas di atas threshold)
        curr_vol = base * VOL_SPIKE_MULT * 2
    ...
    def test_no_spike_normal_vol(self):
        """Vol sekarang = baseline (1.0×) → tidak lolos VOL_SPIKE_MULT."""
        base = 50_000_000.0
        history = self._build_vol_history(base, 10)
        # Vol sekarang = base sendiri → ratio = 1.0, jauh di bawah VOL_SPIKE_MULT (1.5)
        ok, ratio = gate_vol_spike(base, history)
        assert ok is False, f"Expected no spike, ratio={ratio:.2f}"
...
class TestCalcLevels:
    ...
    def test_sl_pct_matches_config(self):
        ...
        lvl = calc_levels(1000.0)
        assert abs(lvl["sl_pct"] - SL_PCT) <= 0.01
...
class TestCalcScore:
    def _make_sig(self, pump_pct, vol_ratio, rsi=0.0, has_rsi=False, snaps=20):
        """
        Bangun sig dict yang konsisten dengan output analyze_pair.
        sig_score dihitung manual mengikuti logika G1/G2/RSI di analyze_pair,
        agar calc_score (yang membaca sig_score langsung) memberikan hasil benar.
        """
        # G1 — pump strength
        if   pump_pct >= 8.0: g1 = 5
        elif pump_pct >= 5.0: g1 = 4
        ...
        # RSI bonus
        rsi_bonus = 1 if (has_rsi and rsi >= 45) else 0
        sig_score = g1 + g2 + rsi_bonus
        return {
            "pump_pct": pump_pct, "vol_ratio": vol_ratio,
            "rsi": rsi, "has_rsi": has_rsi, "snaps": snaps,
            "sig_score": sig_score,
        }

    def test_tier_s_high_all(self):
        # pump=12→g1=5, vol=15→g2=5, rsi+1, snaps≥16→+2 = 13 → tier S
        sig = self._make_sig(pump_pct=12, vol_ratio=15, rsi=58, has_rsi=True, snaps=20)
        score, tier = calc_score(sig)
        assert tier == "S", f"Expected S, got {tier} (score={score})"
        assert score >= 10

    def test_tier_a_plus(self):
        # pump=6→g1=4, vol=9→g2=5, rsi+1, snaps≥8→+1 = 11 → tier S atau A+
        sig = self._make_sig(pump_pct=6, vol_ratio=9, rsi=52, has_rsi=True, snaps=10)
        score, tier = calc_score(sig)
        assert tier in ("A+", "S"), f"Expected A+ or S, got {tier} (score={score})"
        assert score >= 7

    def test_tier_a_minimum(self):
        # pump=3.5→g1=3, vol=5.5→g2=4, snaps=3→+0 = 7 → tier A+
        # Revisi: sig ini masuk A+ bukan A — test disesuaikan
        sig = self._make_sig(pump_pct=3.5, vol_ratio=5.5, snaps=3)
        score, tier = calc_score(sig)
        assert tier in ("A", "A+"), f"Expected A or A+, got {tier} (score={score})"
        assert score >= 4

    def test_score_increases_with_strength(self):
        # weak: pump=1→g1=2, vol=1.5→g2=2, snaps=5→+0 = 4
        # strong: pump=12→g1=5, vol=20→g2=5, rsi+1, snaps=20→+2 = 13
        weak   = self._make_sig(1.0, 1.5, snaps=5)
        strong = self._make_sig(12, 20, rsi=58, has_rsi=True, snaps=20)
        s_weak, _   = calc_score(weak)
        s_strong, _ = calc_score(strong)
        assert s_strong > s_weak, f"strong={s_strong} harus > weak={s_weak}"
...
class TestWrBucketing:
    def _make_buckets(self) -> dict:
        """
        Bucket dummy dengan sample >= MIN_BUCKET_SAMPLE (5) di semua level.
        A+ pump10-20% vol12-20x → WR tinggi, A pump<5% vol<7x → WR rendah.
        """
        WIN_SET = {"TP1", "TP2", "TP3"}
        raw = {}
        data = [
            # Tier A+: pump 10-20%, vol 12-20x → 5 win (100%)
            *[("A+", 11, 14, "TP2")] * 5,
            # Tier A: pump <5%, vol <7x → 1 win, 4 loss (WR rendah)
            *[("A",  3.5, 5.5, "SL")]  * 4,
             ("A",  3.5, 5.5, "TP1"),
        ]
    ...
    def test_high_wr_tier_a_plus(self):
        """Tier S (sig_score=10) dengan pump+vol kuat → bucket S ditemukan + WR tinggi."""
        WIN_SET = {"TP1", "TP2", "TP3"}
        raw = {}
        data = [*[("S", 11, 14, "TP2")] * 5]
        for tier, pp, vr, result in data:
            ...
            for key in [
                ...
                b = raw.setdefault(key, {"n": 0, "n_win": 0})
                b["n"] += 1; b["n_win"] += int(iw)
        buckets = {k: {"wr": round((b["n_win"]+0.5)/(b["n"]+1), 3), "n": b["n"], "n_win": b["n_win"]}
                   for k, b in raw.items()}
        sig = {"pump_pct": 11, "vol_ratio": 14, "rsi": 0, "has_rsi": False,
               "snaps": 5, "sig_score": 10}
        wr, key, has = predict_wr(sig, buckets)
        assert has is True, f"Expected bucket found, key={key}, buckets={list(buckets.keys())}"
        assert wr >= 0.60, f"Expected WR ≥ 60%, got {wr:.0%}"

    def test_low_wr_tier_a(self):
        """Tier A+ (sig_score=7) dengan pump+vol lemah → WR rendah dari bucket."""
        WIN_SET = {"TP1", "TP2", "TP3"}
        raw = {}
        data = [*[("A+", 3.5, 5.5, "SL")] * 4, ("A+", 3.5, 5.5, "TP1")]
        for tier, pp, vr, result in data:
            iw = result in WIN_SET
            for key in [
                f"{tier}|{_pump_bucket(pp)}|{_vol_bucket(vr)}",
        ...
        buckets = {k: {"wr": round((b["n_win"]+0.5)/(b["n"]+1), 3), "n": b["n"], "n_win": b["n_win"]}
                   for k, b in raw.items()}
        sig = {"pump_pct": 3.5, "vol_ratio": 5.5, "rsi": 0, "has_rsi": False,
               "snaps": 5, "sig_score": 7}
        wr, key, has = predict_wr(sig, buckets)
        assert has is True, f"Expected bucket found, key={key}"
        assert wr < 0.55, f"Expected WR < 55%, got {wr:.0%}"

    def test_empty_buckets(self):
        sig = {"pump_pct": 5, "vol_ratio": 8, "rsi": 0,
               "has_rsi": False, "snaps": 5, "sig_score": 0}
        wr, key, has = predict_wr(sig, {})
        assert has is False
        assert wr == 0.0

    def test_fallback_to_coarse(self):
        """Jika fine/medium bucket tidak ada, fallback ke tier saja."""
        # calc_score(sig_score=10, snaps=5→+0) = 10 → tier S
        buckets = {"S": {"wr": 0.75, "n": 10, "n_win": 7}}
        sig = {
            "pump_pct": 11, "vol_ratio": 14,
            "rsi": 0, "has_rsi": False, "snaps": 5,
            "sig_score": 10,
        }
        wr, key, has = predict_wr(sig, buckets)
...
class TestAdaptThresholds:
    def _wr_data(self, wr: float, n: int = 60) -> dict:
        # FASE 4: ADAPT_MIN_SAMPLE = 50, default n=60 agar guard terlewati
    ...
    def test_insufficient_sample_unchanged(self):
        """Di bawah ADAPT_MIN_SAMPLE (50) → threshold tidak berubah."""
        orig_p = PRICE_PUMP_PCT
        # n=3 jauh di bawah ADAPT_MIN_SAMPLE=50
...
# ══════════════════════════════════════════════════════════════════
#  UNIT TESTS — FIX v5.4 (tambahan test untuk semua perbaikan)
# ══════════════════════════════════════════════════════════════════

class TestTradingHoursGuard:
    """Tests untuk SCAN_HOUR_GUARD — FIX 3."""

    def test_scan_hour_constants_valid(self):
        """SCAN_HOUR_START dan SCAN_HOUR_END harus dalam range jam yang valid."""
        assert 0 <= SCAN_HOUR_START <= 23
        assert 1 <= SCAN_HOUR_END   <= 24
        assert SCAN_HOUR_START < SCAN_HOUR_END

    def test_scan_hour_default_daytime(self):
        """Default harus jam 8–22 WIB (bukan 0–24 atau ekstrem lainnya)."""
        # Test default values dari ENV (jika ENV tidak di-set)
        import os as _os
        start = int(_os.environ.get("SCAN_HOUR_START", "8"))
        end   = int(_os.environ.get("SCAN_HOUR_END",   "22"))
        assert start == 8
        assert end   == 22

    def test_scan_hour_guard_toggle(self):
        """SCAN_HOUR_GUARD harus bisa di-disable via ENV."""
        import os as _os
        val = _os.environ.get("SCAN_HOUR_GUARD", "true").lower()
        assert val in ("true", "false")


class TestEarlyBreakoutFix:
    """Tests untuk FIX 4 — early breakout tidak lagi overlap dengan pump signal."""

    def test_early_window_does_not_depend_on_price_pump_pct(self):
        """
        Upper bound early signal harus EARLY_BREAKOUT_PCT + 2.0,
        bukan PRICE_PUMP_PCT * 2 (yang hanya 1.0% saat PRICE_PUMP_PCT=0.5%).
        """
        # Dengan PRICE_PUMP_PCT=0.5, bound lama = 1.0% → sempit sekali
        # Bound baru = EARLY_BREAKOUT_PCT (1.0) + 2.0 = 3.0% → window lebih wajar
        upper_bound_lama = PRICE_PUMP_PCT * 2
        upper_bound_baru = EARLY_BREAKOUT_PCT + 2.0
        # Bound baru harus lebih besar dari bound lama (jika PRICE_PUMP_PCT sangat kecil)
        if PRICE_PUMP_PCT < 1.5:
            assert upper_bound_baru > upper_bound_lama, (
                f"FIX 4: upper bound baru ({upper_bound_baru}) harus > lama ({upper_bound_lama}) "
                f"agar early signal punya window yang wajar"
            )

    def test_check_early_signals_window_width(self):
        """Window early signal harus minimal 1.5% agar tidak terlalu sempit."""
        window_width = 2.0  # fixed window dari FIX 4
        assert window_width >= 1.5, "Window early signal terlalu sempit — risiko false entry"

    def test_early_breakout_pct_greater_than_zero(self):
        """EARLY_BREAKOUT_PCT harus > 0 agar guard berfungsi."""
        assert EARLY_BREAKOUT_PCT > 0


class TestWatchVolHardMin:
    """Tests untuk FIX 2 — WATCH_VOL_HARD_MIN sebagai ENV var, bukan magic number."""

    def test_watch_vol_hard_min_is_configurable(self):
        """WATCH_VOL_HARD_MIN harus terdefinisi dan dalam range yang masuk akal."""
        assert 1.0 <= WATCH_VOL_HARD_MIN <= 3.0, (
            f"WATCH_VOL_HARD_MIN={WATCH_VOL_HARD_MIN} di luar range 1.0–3.0"
        )

    def test_watch_vol_hard_min_leq_watch_vol_build_min(self):
        """
        WATCH_VOL_HARD_MIN harus ≤ WATCH_VOL_BUILD_MIN agar filter konsisten:
        hard floor tidak boleh lebih tinggi dari soft floor ENV.
        """
        assert WATCH_VOL_HARD_MIN <= WATCH_VOL_BUILD_MIN or WATCH_VOL_BUILD_MIN >= WATCH_VOL_HARD_MIN, (
            f"WATCH_VOL_HARD_MIN ({WATCH_VOL_HARD_MIN}) > WATCH_VOL_BUILD_MIN ({WATCH_VOL_BUILD_MIN}) "
            f"— filter akan selalu reject semua pair"
        )

    def test_watch_vol_hard_min_default_value(self):
        """Default WATCH_VOL_HARD_MIN harus 1.8 (tidak berubah dari behavior sebelumnya)."""
        import os as _os
        val = float(_os.environ.get("WATCH_VOL_HARD_MIN", "1.8"))
        assert val == 1.8, f"Default harus 1.8 untuk backward compat, dapat {val}"


class TestFetchSummariesRetry:
    """Tests untuk FIX 6 — fetch_summaries punya retry."""

    def test_fetch_summaries_returns_dict(self):
        """fetch_summaries harus selalu return dict (tidak raise exception)."""
        import unittest.mock as mock
        # Simulasi semua attempt gagal
        with mock.patch("requests.get", side_effect=Exception("network error")):
            result = fetch_summaries()
        assert isinstance(result, dict), "fetch_summaries harus return {} saat gagal, bukan raise"

    def test_fetch_summaries_empty_on_failure(self):
        """Saat semua retry gagal, fetch_summaries return dict kosong."""
        import unittest.mock as mock
        with mock.patch("requests.get", side_effect=ConnectionError("timeout")):
            result = fetch_summaries()
        assert result == {}, f"Harus return empty dict, dapat {result}"


# ══════════════════════════════════════════════════════════════════
#  UNIT TESTS — FIX v5.5 (Tier B label + SCORE_DIST granular)
# ══════════════════════════════════════════════════════════════════

class TestTierBLabel:
    """FIX v5.5 #1 — Tier B harus punya label sendiri, bukan fallback ke TIER A."""

    def test_tier_b_in_tier_labels(self):
        """_TIER_LABELS harus mengandung key 'B'."""
        assert "B" in _TIER_LABELS, (
            "FIX v5.5: Tier B tidak ada di _TIER_LABELS — sinyal score rendah "
            "akan salah ditampilkan sebagai TIER A."
        )

    def test_tier_b_label_is_not_tier_a(self):
        """Label tier B tidak boleh sama dengan tier A."""
        if "B" in _TIER_LABELS:
            _, label_b, _ = _TIER_LABELS["B"]
            _, label_a, _ = _TIER_LABELS["A"]
            assert label_b != label_a, (
                f"Tier B label ('{label_b}') tidak boleh sama dengan tier A ('{label_a}')"
            )

    def test_tier_b_emoji_unique(self):
        """Emoji tier B harus berbeda dari A, A+, S."""
        if "B" in _TIER_LABELS:
            emoji_b = _TIER_LABELS["B"][0]
            for t in ("S", "A+", "A"):
                assert _TIER_LABELS[t][0] != emoji_b, (
                    f"Emoji tier B ('{emoji_b}') sama dengan tier {t} — tidak bisa dibedakan di Telegram"
                )

    def test_all_tier_thresholds_in_tier_labels(self):
        """Semua tier yang digunakan calc_score harus ada di _TIER_LABELS."""
        used_tiers = {"S", "A+", "A", "B"}
        for t in used_tiers:
            assert t in _TIER_LABELS, f"Tier '{t}' tidak ada di _TIER_LABELS — akan fallback ke label salah"

    def test_calc_score_tier_b_returns_correct_label(self):
        """calc_score pada sig dengan score rendah harus return tier B, bukan SKIP."""
        sig = {"sig_score": 1, "snaps": 3}  # score 1 → tier B
        score, tier = calc_score(sig)
        assert tier == "B", f"Score=1 seharusnya tier B, dapat '{tier}'"
        assert "B" in _TIER_LABELS, "Tier B tidak terdefinisi di _TIER_LABELS"
        emoji, label, _ = _TIER_LABELS["B"]
        assert "B" in label.upper(), f"Label tier B ('{label}') harus mengandung 'B'"
