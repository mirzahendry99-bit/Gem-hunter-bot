...
║  Stack     : Indodax Public API + pandas-ta + Telegram           ║
...
def log(msg: str, level: str = "info") -> None:
    """Structured log wrapper dengan level info/warn/error."""
...
def fetch_candles(pair: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        log(f"fetch_candles({pair}): {e}", "warn")
        return None
...
def gate_rsi_cross(df: pd.DataFrame) -> tuple[bool, float]:
    ...
    recent_low = float(min(rsi_vals[-4:-1]))   # min dari 3 candle sebelum candle terakhir
...
def analyze_pair(pair: str, ticker_data: dict) -> Optional[dict]:
    ...
    try:
        ...
        # Pre-filter cepat di level ticker (tidak perlu fetch candle)
        if last_price <= 0:
            return None
