...
WATCH_VOL_BUILD_MIN   = float(os.environ.get("WATCH_VOL_BUILD_MIN",   "1.5"))  # FASE1: 2.0→1.5 (deteksi build-up lebih awal)
...
def detect_pre_pump(
    ...
    try:
        ...
        if vol_ratio < 1.8:
            return None   # 4.4: WATCH hanya untuk build-up signifikan
...
def run_scan() -> None:
    ...
    log("=" * 64)
    log("🔍 INDODAX PUMP SIGNAL MONITOR v5.3-FINETUNE (FASE 5 — TIER LOCK & LIFECYCLE)")
