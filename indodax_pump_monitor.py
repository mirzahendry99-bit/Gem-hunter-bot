...
def probe_candle_api() -> bool:
    """
    Diagnosa API candle Indodax sebelum scan dimulai.
    Coba fetch candle BTC/IDR (paling liquid) dengan beberapa format URL/param.
    Log response aktual untuk debugging.
    Return True jika ada format yang berhasil.
    """
    now_ts  = int(time.time())
    from_ts = now_ts - (200 * 15 * 60)   # ~50 jam ke belakang

    candidates = [
        # Format 1: symbol BTCIDR (no underscore, uppercase) + countback
        {
            "url": INDODAX_TV_HISTORY,
            "params": {"symbol": "BTCIDR", "resolution": "15",
                       "from": from_ts, "to": now_ts, "countback": 10},
            "label": "TV_BTCIDR_countback",
        },
        # Format 2: tanpa countback
        {
            "url": INDODAX_TV_HISTORY,
            "params": {"symbol": "BTCIDR", "resolution": "15",
                       "from": from_ts, "to": now_ts},
            "label": "TV_BTCIDR_no_countback",
        },
        # Format 3: symbol lowercase
        {
            "url": INDODAX_TV_HISTORY,
            "params": {"symbol": "btcidr", "resolution": "15",
                       "from": from_ts, "to": now_ts},
            "label": "TV_btcidr_lower",
        },
        # Format 4: symbol dengan separator IDR/BTC style
        {
            "url": INDODAX_TV_HISTORY,
            "params": {"symbol": "BTC/IDR", "resolution": "15",
                       "from": from_ts, "to": now_ts},
            "label": "TV_BTC/IDR",
        },
    ]

    log("🔬 Probe API candle Indodax (BTC/IDR test)...")
    for c in candidates:
        try:
            resp = requests.get(c["url"], params=c["params"], timeout=REQUEST_TIMEOUT_SEC)
            body = resp.content.strip()
            snippet = body[:120].decode("utf-8", errors="replace") if body else "(empty)"
            log(f"  [{c['label']}] HTTP {resp.status_code} | body: {snippet}")

            if resp.status_code == 200 and body:
                try:
                    data = resp.json()
                    if data.get("s") == "ok" and len(data.get("t", [])) > 0:
                        log(f"  ✅ Format '{c['label']}' BERHASIL — {len(data['t'])} candle diterima")
                        return True
                except Exception:
                    pass
        except Exception as e:
            log(f"  [{c['label']}] exception: {e}", "warn")

    log("  ❌ Semua format API candle gagal — tidak ada data OHLCV dari Indodax TradingView endpoint.", "error")
    return False
...
def fetch_candles(pair: str) -> Optional[pd.DataFrame]:
    ...
    # Coba tanpa 'countback' terlebih dahulu (lebih kompatibel)
    params = {
        "symbol":     symbol,
