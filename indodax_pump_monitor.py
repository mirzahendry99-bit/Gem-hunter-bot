# indodax_pump_monitor_f5c.py — v5.4-FINETUNE
...
# FASE 1 UNBLOCK: semua gate sub-threshold dilonggarkan.
# Breakout lebih mudah dicapai, sniper path lebih lebar.
...
WATCH_PRICE_FLAT_PCT  = float(os.environ.get("WATCH_PRICE_FLAT_PCT",  "3.0"))  # FASE1: 2.0→3.0 (lebih toleran pergerakan harga kecil)
WATCH_VOL_BUILD_MIN   = float(os.environ.get("WATCH_VOL_BUILD_MIN",   "1.8"))  # FIX: dinaikkan ke 1.8 agar konsisten dengan WATCH_VOL_HARD_MIN (floor tidak boleh > soft min)
WATCH_VOL_BUILD_MAX   = float(os.environ.get("WATCH_VOL_BUILD_MAX",   "4.9"))
WATCH_MIN_SNAPS       = int(os.environ.get("WATCH_MIN_SNAPS",          "3"))   # FASE1: 5→3 (lebih cepat aktif)
MAX_WATCH_PER_RUN     = int(os.environ.get("MAX_WATCH_PER_RUN",        "5"))   # FASE1: 3→5 (lebih banyak WATCH terkirim)
# Hard floor vol_ratio untuk WATCH — pair dengan build-up di bawah ini tidak masuk WATCH.
# Turunkan ke 1.5 jika WATCH terlalu sepi; naikkan ke 2.0+ jika terlalu banyak noise.
WATCH_VOL_HARD_MIN    = float(os.environ.get("WATCH_VOL_HARD_MIN",    "1.8"))  # FASE4: hardcoded 1.8 → ENV
...
def run_scan() -> None:
    ...
    log("=" * 64)
    log("🔍 INDODAX PUMP SIGNAL MONITOR v5.4-FINETUNE (FASE 5 — TIER LOCK & LIFECYCLE)")
